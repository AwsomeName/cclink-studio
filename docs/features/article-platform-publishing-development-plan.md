...
> 文档版本：1.4
> 状态：M1-M3 与发布 Runtime 系统性收敛已在当前工作树实现并通过自动工程门禁；正式版本交付、
> 真实 CSDN 闭环与真人验收待完成
> 最后更新：2026-08-30
...
截至 v0.1.72，M1 和 M2/M3 的基础结构已经进入生产代码；当前工作树另已完成 Runtime 系统性收敛，不能再把下列能力写成“尚未实现”：
...
- **v0.1.72 已发布基线仍存在假运行阻塞；当前工作树已完成系统性工程修复。**修复包括 main-owned
  启动、generation/owner identity、统一收敛、一次性副作用、双租约待核验、启动修复和用户恢复入口。
  自动门禁与真实 Electron/CDP smoke 已通过，但必须完成正式版本交付和真人复验后才能改变已发布
  用户的结论。
