...
| Workbench Tab        | 重命名、固定/解除固定、复制、关闭其他、关闭右侧、关闭                    | 恢复关闭项                         | 关闭草稿继续使用现有保存确认           |
...
| 运营/生产            | 打开配置、生成草稿、查看状态、复制诊断                                 | 模板与批量准备                       | 目标动作需有界授权；特殊卡点转人工     |
...
> 状态：已完成。`docs/ops/context-action-m4-acceptance.md` 的 H1-H5 已通过。
...
- `ContextTarget` 已增加数据源、集合、保存查询、记录、运营平台、生产、Android 和设置项；各领域通过自己的 command/contribution 模块注册，没有修改 `ContextMenuHost`。
- 数据源菜单只提供只读查询、稳定标识复制、记录 JSON 复制和挂载到 Agent；配置端点、凭证和密钥不进入复制命令。
- 运营菜单只准备站点会话、打开本地配置和复制状态；生产菜单只扫描、检查、生成本地报告和复制状态；两者没有发布、发送、下单或支付命令。
- Android 菜单复用现有设备 surface，在 adb 或设备不可用时显示具体原因；设置菜单只允许重置单项安全配置和复制非敏感 setting key，凭证输入行不绑定设置项菜单。
- 单元测试覆盖领域危险命令缺失、领域 owner 调用、Android 缺少 adb、数据源/设置复制边界、设置状态所有者和生产能力降级。
- 2026-07-22 当前 M4 工作树自动门禁：`pnpm verify` 通过（154 files / 932 tests），`pnpm smoke:standalone` 通过（local 9/9、UI 6/6、workflow 8/8、restore 4/4）。workflow 已实际验证运营、生产和设置的鼠标右键与 Shift+F10 路径。
- H1-H5 已由用户统一确认通过：数据源、运营/生产、Android 降级、设置安全边界和跨模块隔离均无阻断问题；M4 功能边界关闭。

### M5：一致性、可访问性与退出验收

> 状态：已完成；`docs/ops/context-action-m5-acceptance.md` 的 H1-H5 已通过。
...
- 已建立并维护区域/目标/owner 库存；当前 21 种 `ContextTarget` 均有明确 contribution 覆盖或产品结论。
- catalog 启动校验会拒绝重复 command/contribution、孤立 contribution、非法排序以及未覆盖 target；单个 contribution 构建失败被隔离并写入诊断，不会拖垮整个 Host。
- 新增 AST 静态门禁，当前只允许 17 个 renderer 触发 owner、1 个 Browser 原生菜单 owner 和 1 个 ContextMenu Store owner；遗留全局菜单文件重新出现会直接失败。
- 键盘支持方向键、Home/End、Tab/Shift+Tab、Enter/Space 和 Escape；初始焦点在可见帧前建立，禁用项暴露明确原因，菜单在紧凑视口内自动避让。
- 工作台状态诊断已纳入最近 50 条脱敏上下文操作失败，可区分构建失败、陈旧目标、权限拒绝和领域执行失败。
- 2026-07-22 焦点时序修正提交 `c2d8262` 已在全新 detached worktree 通过 `pnpm install --frozen-lockfile`、`pnpm verify`（155 files / 937 tests）和 `pnpm smoke:standalone`（local 9/9、UI 6/6、workflow 9/9、restore 4/4）；远端 CI run `29905998042` 的 verify 和 smoke job 均通过。
- 用户接受简化验收，H1-H5 结合 workflow、诊断脱敏、stale-target、contribution 隔离以及 M3 既有人工回归证据全部通过；M5 功能边界关闭，验收记录提交后的最新 HEAD 仍须完成最终干净门禁与远端 CI 后才可合并。
...
M5 关闭前不得扩大菜单功能面；后续新增区域必须先更新库存，并通过 catalog 与 owner 静态门禁。


### 标签固定与项目恢复（2026-09-22）

固定状态为 Tab descriptor 的可选布尔字段 `pinned`，由现有 TabModel / WorkspaceState 保存。
固定标签靠前排列，保留标题，显示固定标记并隐藏关闭按钮；右键单独关闭和关闭当前标签快捷键仍可用。
批量关闭其他/右侧标签跳过固定标签，拖动只能在同一固定分组内排序。
固定不改变工作空间归属，也不自动保存网站账号。设置等全局临时页保持既有生命周期，不增加重启恢复。
普通网页的账号草稿引用与 Profile 随标签恢复；网站资源服务保留 open 草稿，保存中断回到 open，
只对 cleanup-pending 记录重试清理。用户主动关闭最后一个同草稿标签仍走既有清理入口。
