...
1. 打开包含多张大图片的 Markdown 时，图片按预算和可见性加载；超限资源显示降级提示，
   不冻结工作台。

### A9：排他新建文件

1. 工作空间中已有一个包含内容的 `notes.md`。
2. 用户在同一目录新建同名 `notes.md`。
3. Studio 明确提示目标已存在；原文件内容逐字不变，也不生成错误的新 Tab 或草稿。

只有 A1-A9 在真实应用中通过，且相关自动化门禁通过，才可以恢复“稳定完成”声明。
...
pnpm exec vitest run \
  ...
  src/renderer/src/stores/fs-store.test.ts \
  src/main/ipc/ipc-contract.test.ts
pnpm typecheck
pnpm verify
```

结果为 4 个测试文件、120 项测试全部通过，renderer 与 node TypeScript 检查通过；新增回归覆盖
保存关闭期间继续编辑和同名排他创建。完整 `pnpm verify` 同时通过：284 个测试文件，1656 项
...
### P0-3（已最小修复）：保存并关闭期间产生的新编辑可能被销毁

证据：`editor-store.saveFile()` 会保留保存请求发出后的新内容并重新标记 `dirty=true`，但关闭
...
证据：文件树“新建文件”此前调用普通 `writeFile(newPath, '')`，Node 默认会截断已存在目标。
...
用户结果：指定保存不会写错文件；冲突或保存期间产生新编辑时不会关闭并丢弃草稿；新建同名
文件不会清空已有内容。
...
退出条件：A3、A5、A7、A9 通过；P0 测试失败时阻止 `pnpm verify`。
...
退出条件：A1、A8 通过；长文档和大资源达到明确性能预算。

### M6：综合真实验收与完成声明恢复

用户结果：A1-A9 在正式构建或等价真实应用中全部通过。

工程任务：

- `pnpm verify` 与受影响 smoke 全绿；
- 真实应用执行 A1-A9，并保存版本、平台和结果；
...
- 如果 M1 仍需要大规模重写才能封闭 P0，应先做最小精确目标检查和关闭结果检查，不得让
  P0 等待完整 Session 架构。
- 如果 DocumentSession 迁移超过 60 分钟仍未产生新的 A1-A9 可验收增量，停止横向抽象，
  回到单文档打开、编辑、保存纵向闭环。
...
- 如果只有单元测试绿色而 A1-A9 未执行，只能报告工程准备度。
