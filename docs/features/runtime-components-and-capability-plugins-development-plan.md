...
> 状态：已暂停，不属于当前开发路线图。本文仅保留为未来通过重启门禁后的参考方案。最后更新：2026-08-15。
> 产品事实源：`runtime-components-and-capability-plugins.md`。
> 本计划不取代 `desktop-update-development-plan.md`；完整 App 更新仍是核心发布前置能力。

## 1. 结论

方案技术可行，但属于中等偏大的架构调整，当前收益不足，决定暂停。固定版本 Runtime 安装、修复和 App 替换复用已经获得主要用户价值；不再为了理论上的 npm 解耦继续建设签名目录、多版本回滚和通用 Plugin Host。

重启本计划必须同时满足：

1. 至少两个真实 Runtime 版本在当前 Host/SDK 上通过真实兼容验收，或至少一个 Provider/Adapter/MCP 出现持续独立发布需求。
2. 最近十次实际发版中，至少三次能由该边界解决，而不是仍需修改 UI、IPC、SDK 或核心代码。
3. 提交新 ADR，重新确认收益、信任根、兼容矩阵、回滚和运维成本。

第一版不做公共市场、不运行 npm CLI、不支持任意包、不加载 Node 插件、不注入 React UI，
也不实现运行中无感热替换。插件或 Runtime 更新完成后允许重启插件宿主或 Studio 生效。

以下架构、里程碑和工作量均为冻结参考，不得据此自动开工或报告当前产品进度。

## 2. 用户现在能做什么、还不能做什么

### 用户现在能做什么

- 从组件页安装、检查、修复和卸载固定版本 Claude、OCCT、scrcpy 和 agent-device Helper。
- 使用已安装的 managed Claude、OCCT 和 scrcpy，并在替换 App 后继续复用。
...
- 在不更新 App 的情况下把 Claude、CAD、scrcpy 切换到新版本。
- 在设置中审计插件权限、来源、兼容版本和健康状态。
- 在坏版本启用失败后自动回到上一已知可用组件。

固定版本 Runtime 管理已有真实用户能力；本计划中的通用插件和真实双版本更新仍为未交付且已暂停。
...
3. 没有合格候选时不得重启 M3；本地 fixture 只能证明工程门禁，不能冒充产品完成。
...
若未来重启，单人完整路线历史估算约 34-50 工程日，即约 7-10 周；首个可验收插件闭环约 3-4 周。该估算已冻结，不是当前计划或排期，也不包含许可、Apple 签名公证、npm/GitHub 发布审批和真人等待时间。
