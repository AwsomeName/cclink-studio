...
展开 Git 浮层，承载变更、仓库、分支、上游以及“提交或推送”入口。
...
3. 用户点击 Git 状态，浮层显示仓库、分支、上游和变更摘要。
4. 用户点击“变更”，看到 staged、unstaged、untracked、conflicted 分组；点击文件看到只读
   Diff 或可操作降级说明。
5. 用户进入“提交”，看到本次将提交的文件清单和数量，填写提交信息并明确确认。
...
- 在状态栏看到真实分支、变更数、增删行数和上游状态。
- 在提交前看到变更文件清单或 Diff。
- 选择要提交的文件、保留已有 staged 意图或填写正常开发提交信息。
- 把“提交”和“Push”作为两个可理解、可恢复的操作执行。
- 从统一 Git 入口处理未配置远程、ahead/behind、detached HEAD 或冲突状态。

现有手动备份文档仍描述当前实现事实。本文件描述已经确认的目标产品逻辑，不得在代码和
真人验收完成前把目标状态写成已交付。
...
| 有 staged/unstaged/untracked 变更              | 显示“提交…”并打开提交确认面板                     |
...
| 浮层开关、临时选择和提交信息草稿  | renderer Git store，可丢弃                  |
...
下一步最该做的是先交付只读的状态栏、浮层、变更清单和 Diff 闭环，并用真实当前仓库验证
计数；在读模型稳定前，不接 commit、Push 或远程写操作。
