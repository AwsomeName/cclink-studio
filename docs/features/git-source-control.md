...
窗口背景点击不关闭。无草稿时 Esc/关闭按钮可以关闭；提交信息或文件选择已变化时，Esc/关闭
必须进入“继续编辑 / 放弃草稿”确认。Commit 或 Push 执行期间禁止关闭。工作空间切换不能把
旧草稿提交到新工作空间；过期 snapshot 必须停止动作并要求用户接受最新状态。
...
- 操作窗口背景点击不关闭；脏草稿关闭前确认，写操作执行中禁止关闭。
...
当前最该修正的交互债务是把变更、提交和 Push 从易误关的小浮层迁入统一操作窗口，并验证
草稿防丢和 Commit 成功/Push 失败的部分成功路径。完成后再补齐 G4 的显式 Fetch、远程关联
和真实 HTTPS/SSH 失败验收；旧建仓入口不能重新成为 Git 状态事实源。
