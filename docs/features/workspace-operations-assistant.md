...
> `docs/features/ai-web-affairs-agent-development-plan.md` 为准。目标“网站与账号”只管理
> 当前项目，侧栏资源直接打开实际 Browser Tab；本文中的 Profile、默认平台和操作会话
> 只属于待迁移的历史实现，不再定义目标产品交互。

## 结论

工作空间运营第一版不做独立的“运营平台”、不做“每个平台一个工作区”、不做完整账号管理器。
...
├─ 文案会话：读取工作空间资料，写 Markdown
└─ 平台操作会话：打开浏览器，读取账号配置，填表并等待确认提交
```

工作空间仍然是本地目录。平台不是工作空间，平台只是工作空间配置文件里的一条
记录。会话负责执行任务，产物落回工作空间文件。

## 用户故事

用户打开 `cclink` 工作空间后，可以对 CCLink Studio 说：

```text
根据工作空间资料写一版公众号内测宣发稿，保存到 docs/公众号首发稿.md。
```
...
- 将发布结果写回工作空间 Markdown，例如 `docs/发布记录.md`。
...
- 密码、Token、恢复码等秘密信息不进入工作空间可见文件；后续如支持第三方凭证，只保存统一 `CredentialService` 的稳定引用。
...
目标：让 Agent 围绕当前工作空间资料生成 Markdown 文案。
...
- [x] 能创建绑定当前工作空间的文案工作会话，并预填读取工作空间资料、改写 Markdown 的任务说明。
- [x] 所有产物都落在工作空间目录，不落入全局数据库。
...
- 新增运营服务和 preload API，负责 `cclink-accounts.json`、文案草稿和发布记录写回。当前代码名仍为 `ProjectOpsService` / `projectOps`，属于待迁移兼容命名。
- 工作空间侧栏新增“工作空间运营”区，只在本地工作空间显示。
- 可一键创建 `cclink-accounts.json` 模板。
- 兼容读取受控迁移模块声明的旧账号文件路径，但新建和文档约定统一使用工作空间根目录 `cclink-accounts.json`。
- 可按平台创建文案草稿和文案工作会话。
...
- 不做多账号批量营销。
- 不做平台全覆盖模板市场。
- M7 运营助手自身不实现独立定时任务系统；后续跨能力定时任务统一遵守
  `docs/features/scheduled-tasks.md`，不得在运营模块内建立第二套调度 owner。
- 不做完整社媒管理后台。
...
如果第一版把“平台”做成工作区，会污染工作空间模型；平台只是工作空间里的配置。
...
1. 在 `cclink` 工作空间里写一篇宣发文案。
2. 用平台操作会话打开目标平台。
3. Agent 填好内容。
4. 用户确认提交。
5. 发布结果写回工作空间文档。

这五步跑通，CCLink Studio 就开始真正服务工作空间运营；跑不通，再漂亮的平台模型都没意义。
