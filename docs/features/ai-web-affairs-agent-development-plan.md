...
> 文档版本：2.2
> 基线状态：全局“网站与账号”、运营矩阵和项目事务引用已进入统一验收；Agent 只读账号
> 目录已实现。Agent 使用指定账号打开登录环境、操作网页、人工接管和事务回写的产品决定
> 已确认，但尚未实现；当前执行路径继续 fail-closed。
> 最后更新：2026-08-18
...
| 维度         | 当前事实                                                                   |
| ------------ | -------------------------------------------------------------------------- |
| 用户功能进度 | 全局账号、跨项目手动打开、运营矩阵、项目事务引用和 Agent 只读目录已实现    |
| 工程准备度   | v3 迁移、全局 contract、安全元数据投影、矩阵 revision 和执行负向门禁已实现 |
| 当前阶段     | G1–G5/T1 `Acceptance`；R1 `In Progress`；X0 `Ready`；X1–X5 `Pending`       |
| 当前阻塞     | 无产品决策阻塞；Agent 账号执行仍未实现，当前路径保持 fail-closed           |
| 下一用户增量 | X1：Agent 按用户指令打开指定账号的正确可见 Browser Tab                     |
| 完成声明     | 全局账号实现已进验收；Agent 执行尚未开工，不得声明对应能力完成             |
...
| X0     | Agent 账号执行契约门禁     | Ready       | G5、ADR 0016 | 无新增用户能力；固定唯一执行链和安全边界 | 本文第 16 节     |
| X1     | Agent 打开指定账号         | Pending     | X0           | 说出账号后打开正确登录环境和可见 Tab     | 本文第 16 节     |
| X2     | 真实网页只读查询           | Pending     | X1           | Agent 查询进度、原因并返回证据           | 本文第 16 节     |
| X3     | 通用网页办理准备           | Pending     | X2           | Agent 填写、上传、下载并保存草稿         | 本文第 16 节     |
| X4     | 人工接管与敏感动作阻断     | Pending     | X3           | 人可接管、交还，关键动作由用户完成       | 本文第 16 节     |
| X5     | 事务回写、等待和恢复       | Pending     | X4、T1       | 跨运行持续跟踪并把证据写回事务           | 本文第 16 节     |
...
当前用户还不能让 Agent 使用登记账号执行网页任务；已有 `web_accounts_list` 只解决目录
读取。X1–X5 全部完成并通过真人真实网站验收后，才能声明“Agent 可以接管网页事务”。
...
BrowserTask：固定 accountId/tabId/profileId/conversationId/agentRunId 并执行网页动作
       ├─ 普通动作：读取、点击、填写、上传、下载
       └─ 敏感边界：PermissionManager 阻断并转人工接管
              ↓
...
| 敏感动作阻断              | `PermissionManager`                | 当前动作转人工接管     | Agent 绕过阻断执行最终动作          |
...
- **状态**：Pending；X4 和 T1 完成后进入 Ready
