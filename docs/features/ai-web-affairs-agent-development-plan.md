...
> 目录已实现，打开和操作全局账号仍未决定，当前 UI 和执行路径均 fail-closed。
> 最后更新：2026-08-18
...
但是否可以打开或操作这些账号暂未决定。本轮不得把 MCP 只读目录扩张为 BrowserTask、
Attempt 或定时任务的执行权限。
...
| 当前阶段     | E0 `Complete`；G1–G5、T1 `Acceptance`；R1 `In Progress`                    |
| 当前阻塞     | 无工程阻塞；AI 打开和操作账号是明确未决项，不阻塞 Agent 读取登记目录       |
| 下一用户增量 | 使用真实测试账号完成跨项目登录身份与重启 Session 验收                      |
| 完成声明     | 可声明实现进入验收；真人验收前不得声明产品里程碑 Complete                  |
...
| A2–A4  | AI 执行、交接、等待和动态流 | Pending     | 独立产品决定 | 暂不新增；等待 AI 账号调用方案确认       | 本文后续复审     |
...
- 用户仍只能使用现有项目账号 v2 实现；全局列表、跨项目同 Profile/Session 和运营矩阵尚未
  实现，不能按新方案验收。
- 事务仍属于当前项目，既有三段式 Tab 和工程组件保留；AI 是否可以调用全局账号暂未决定。
...
- A1 需要在全局引用上回归；A2–A4 保持 `Pending`，等待 AI 账号调用独立产品决定。
...
- A2–A4 不作为本轮账号全局化的完成条件；AI 调用账号继续单独决策。
...
- 打开、登录、自动选号、网页操作和跨事务执行授权继续是独立产品决定。
