...
用户现在已有“网站与账号”的第一条实现切片：可以录入任意网站、业务主体、账号标签
和 Browser Profile，并从列表用对应 Profile 打开网站、查看脱敏会话诊断。该切片已
通过 Electron UI smoke，但尚未完成真人登录和应用重启验收；持久事务、动态流程、
三段式事务 Tab、人工交还和外部等待仍未实现。

当前项目状态：

| 维度         | 当前事实                                                           |
| ------------ | ------------------------------------------------------------------ |
| 用户功能进度 | W1 首条切片已实现，尚未完成登录、重启、双 Profile 和旧配置迁移验收 |
| 工程准备度   | WebResource contract/store/service/IPC 已落地并通过定向测试与构建  |
| 当前阶段     | `E0` 与 `W1` In Progress                                           |
| 当前阻塞     | 无技术阻塞；缺真实登录/重启验证、旧配置迁移和资源详情 Tab          |
| 下一用户增量 | 完成 W1 真人闭环：未预置网站登录、重启恢复、双 Profile 不串号      |
| 完成声明     | 只能声明首条实现切片和工程门禁通过，不能声明 W1 或产品能力完成     |
...
| ------ | ---------------------- | ----------- | ----- | ---------------------------------------- | --------------------------------------------- |
| E0     | 契约和存储冻结         | In Progress | 无    | 无新增用户能力                           | 评审记录、contract/schema 测试                |
| W1     | 网站与账号闭环         | In Progress | E0    | 添加任意网站和多主体账号，登录并重启恢复 | `docs/ops/ai-web-affairs-agent-acceptance.md` |
| A1     | 持久事务和三段式 Tab   | Pending     | W1    | 创建事务、关联资源、查看流程和节点详情   | 同上                                          |
| A2     | 真实网页节点和人工交接 | Pending     | A1    | 让 AI 执行节点、接管、交还并重新验证     | 同上                                          |
...
| 用户功能 | W1   | In Progress | 任意网站、主体和账号可以保存、登录、重启恢复  | 5–8 人日  |
| 用户功能 | A1   | Pending     | 可以创建事务并使用三段式 Tab 管理资源和流程   | 6–9 人日  |
