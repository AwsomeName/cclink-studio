...
> 文档版本：1.2
> 基线状态：实施中；W1 真人验收经用户批准延期，A1 第一纵向切片已落地
> 最后更新：2026-07-31
...
用户现在不仅可以在“网站与账号”中管理任意网站、主体、账号和 Browser Profile，
还可以从独立“事务”入口创建一件持久网页事务，关联主体、账号和本地材料引用，以分行
步骤生成五节点等线性 DAG，并在通用三段式事务 Tab 中查看资源、整体流程和节点详情。
用户可以人工推进节点；完成上游节点会解锁下游，完成节点和追加时间线不会被覆盖，应用
重启后事务及进度仍存在。AI Attempt、人工接管、外部等待复查、流程结构编辑和材料有效
性诊断仍未实现。W1 真实网站登录验收经用户明确批准延期，但未被标记为通过或完成。

当前项目状态：

| 维度         | 当前事实                                                       |
| ------------ | -------------------------------------------------------------- |
| 用户功能进度 | W1 自动化通过；A1 已形成创建、查看、推进和重启恢复的第一闭环   |
| 工程准备度   | WebResource 与 WebAffair contract/store/service/IPC 已通过门禁 |
| 当前阶段     | `E0` In Progress；`W1` Acceptance（延期）；`A1` In Progress    |
| 当前阻塞     | 无技术阻塞；W1 真人登录验收按用户授权延期，不阻塞 A1 手工事务  |
| 下一用户增量 | 编辑未执行节点和依赖，并诊断本地材料缺失或变化                 |
| 完成声明     | 可声明 A1 第一切片和自动化门禁通过，不能声明 A1、W1 或产品完成 |
...
| A1     | 持久事务和三段式 Tab   | In Progress | W1    | 创建事务、关联资源、查看流程和节点详情   | 同上                                          |
