...
> 架构决定：`docs/decisions/0014-global-web-account-catalog.md`、
...
用户现在可以使用“先登录、后保存”流程只保存一次全局账号，在不同项目看到并手动打开同一
`accountId` 和 Profile。用户也可以管理版本化运营矩阵，并让项目事务保存全局账号或矩阵快照。

当前目标已经冻结为：网站、主体、账号、登录状态和运营矩阵由 `WebResourceService` 全局
管理；Browser Tab 仍属于当前工作空间；事务、材料、流程和证据继续由 `WebAffairService`
按 `workspaceId` 隔离。事务可以保存全局账号或矩阵引用；Agent 可以读取账号安全元数据。
ADR 0016 已确认 Agent 在用户明确任务中可以使用显式账号打开登录环境并执行网页操作，
敏感边界转人工确认或接管；该能力必须按 X0–X5 独立交付，不能把只读目录直接改成执行权。

未保存关闭、保存重试、幂等、草稿清理和独立 Profile 工程能力继续保留；`WebAccount.projectId`、
项目级 Snapshot、“保存到当前项目”文案和跨项目拒绝逻辑已从当前实现移除。v1/v2 迁移保留旧账号、
Profile 和事务引用，疑似重复记录只能由用户确认合并。

事务在全局账号和矩阵快照上的工程回归已完成，但尚未经过真人使用。任何单测、smoke 或历史
Acceptance 都不能替代新方案的跨项目真人验收。

当前项目状态：

| 维度         | 当前事实                                                                   |
| ------------ | -------------------------------------------------------------------------- |
| 用户功能进度 | Agent 可打开指定账号、受控操作网页、人工交接，并从事务节点启动和回写       |
| 工程准备度   | 可信启动桥、账号任务互斥、秘密负向、敏感动作拦截和重观察门禁已实现         |
| 当前阶段     | G1–G5/T1、X0–X5 `Acceptance`；R1/R2 `In Progress`                          |
| 当前阻塞     | 无工程阻塞；缺两个真实账号、真实登录网页和一件真实测试事务的真人验收       |
| 下一用户增量 | 不再扩功能；按 R2 依次完成真实打开、查询、草稿、接管、等待和重启验收       |
| 完成声明     | 代码闭环已进入统一验收；R2 未通过前不得宣称网页事务代理能力 Complete       |
...
| 里程碑 | 名称                       | 状态        | 依赖         | 用户完成后能做什么                       | 计划事实源       |
| ------ | -------------------------- | ----------- | ------------ | ---------------------------------------- | ---------------- |
| E0     | 架构与迁移门禁             | Complete    | 无           | 无新增用户能力；v2 数据具备安全迁移条件  | 全局账号专项计划 |
| G1     | 全局账号列表与一次添加     | Acceptance  | E0           | 一个账号只添加一次，所有项目都能看到     | 同上             |
| G2     | 跨项目 Tab 与登录态复用    | Acceptance  | G1           | 各项目复用同一 Profile/Session           | 同上             |
| G3     | 旧账号迁移、疑似重复与归档 | Acceptance  | G2           | 升级不丢账号、登录态和事务引用           | 同上             |
| G4     | 全局运营矩阵               | Acceptance  | G3           | 一次配置账号集合，所有项目复用           | 同上             |
| T1     | 项目事务引用兼容           | Acceptance  | G3、G4       | 事务仍属项目并引用全局账号/矩阵          | 同上             |
| G5     | Agent 只读账号目录         | Acceptance  | G1、G4       | Agent 能说明已登记的平台、账号和矩阵     | 同上             |
| R1     | 全局账号统一真实 App 验收  | In Progress | G1–G5、T1    | 完成跨项目账号、矩阵、迁移和事务引用闭环 | 同上             |
| X0     | Agent 账号执行契约门禁     | Acceptance  | G5、ADR 0016 | 无新增用户能力；固定唯一执行链和安全边界 | 本文第 16 节     |
| X1     | Agent 打开指定账号         | Acceptance  | X0           | 说出账号后打开正确登录环境和可见 Tab     | 本文第 16 节     |
| X2     | 真实网页只读查询           | Acceptance  | X1           | Agent 查询进度、原因并返回证据           | 本文第 16 节     |
| X3     | 通用网页办理准备           | Acceptance  | X2           | Agent 填写、上传、下载并保存草稿         | 本文第 16 节     |
| X4     | 人工接管与敏感动作阻断     | Acceptance  | X3           | 人可接管、交还，关键动作由用户完成       | 本文第 16 节     |
| X5     | 事务回写、等待和恢复       | Acceptance  | X4、T1       | 跨运行持续跟踪并把证据写回事务           | 本文第 16 节     |
...
> 本节至第 13 节保留 2026-08-05 项目账号方案的工程细节和验收方法，用于 v2 迁移与事务
> 能力复用。凡是“账号属于项目”“只显示当前项目账号”“保存到当前项目”“AI 复用同项目
> `resolveLaunch`”的表述，均已由 ADR 0014 和
> `docs/features/global-web-accounts-development-plan.md` 取代，不再是当前施工要求。
...
| R-18 | 账号全局化后旧 AI 路径意外获得跨项目调用范围    | 极高   | 中     | ADR 0014、负向门禁、AI 路径 fail-closed      | Open |
| R-19 | v2 疑似重复账号被错误合并并丢失登录环境         | 极高   | 中     | 保留全部 ID/Profile，必须人工选择主账号      | Open |
| R-20 | 运营矩阵变更静默扩大既有事务账号范围            | 极高   | 中     | 矩阵 revision 与事务绑定快照                 | Open |
| R-21 | 同一全局账号在多项目手动操作产生页面状态冲突    | 高     | 中     | 首版记录残余风险，不擅自决定并发策略         | Open |
...
| 2026-08-04 | D-09 | “网站与账号”只管理当前项目                  | 用户心智明确，避免全局资源库和项目归属黑盒 | Superseded by ADR 0014  |
| 2026-08-04 | D-10 | 侧栏资源主要点击直接打开实际 Browser Tab    | 网站账号必须是可执行资源，不是静态详情     | Accepted                |
| 2026-08-04 | D-11 | 事务和 AI 复用侧栏同一资源 Tab 启动语义     | 防止 URL、Profile 和当前 Tab 猜测导致串号  | Partially superseded    |
| 2026-08-04 | D-12 | 项目归属保存稳定 projectId                  | 项目移动不失联，复制项目不继承原归属       | Superseded for accounts |
...
| 2026-08-18 | D-16 | 账号全局唯一、事务仍归项目、AI 调用暂不决定 | 消除跨项目重复配置且不提前扩大 AI 权限     | ADR 0014 Accepted       |
...
### 2026-08-14 · W1-A 真人验收发现保存空值无反馈

用户现在能做什么、还不能做什么：

- 修复后，用户展开保存区时会看到根据当前页面标题或域名自动预填的账号名称；即使清空
  名称后点击保存，也会得到明确错误，不再出现“按钮看似可点但没有任何反应”。
- 保存成功会显示成功 Toast、把当前 Tab 原地转为正式资源，并通知侧栏重新读取项目资源。
- 真实 `0.1.32` 现场已经证明旧交互失败；新代码已经通过受影响自动门禁，但尚未完成新
  版本真人复测，因此 W1-A 仍是 `Acceptance`，不能声明问题已验收关闭。

问题证据与根因：

- 2026-08-14 现场截图中账号名称输入为空，`0.1.32` 的保存按钮因空值被禁用；renderer
  保存函数还有同样的空值静默返回守卫。
- 同期框架诊断没有保存 IPC 或存储错误，符合保存请求没有进入主进程，而不是落库后侧栏
  丢失。
- 原 UI 没有必填说明，禁用按钮仍保留蓝色主按钮外观，点击后没有行内提示或 Toast；这是
  W1-A 用户闭环缺陷，不是用户操作错误。

本次修正：

- 从当前页面标题预填账号名称，标题不可用时退回到去除 `www.` 的域名；无有效网页时不
  猜测。
- 空值保存不再静默失效：按钮只在保存请求进行中禁用，空值点击显示“请输入账号名称”。
- 保存拒绝、异常和成功均显示 Toast，并记录不包含账号名称、Cookie 或登录内容的结构化
  renderer 日志；保存后侧栏刷新异常也进入框架日志。
- UI smoke 增加“自动预填—清空—明确报错—重新填写—侧栏出现”的纵向断言；纯函数测试
  覆盖页面标题、域名回退和无效地址。

验证证据与下一步：

- 定向测试 4 个文件、25 项通过；web 类型检查和受影响文件 Lint 通过。
- `pnpm smoke:ui` 12/12，通过实际 Electron UI 验证名称自动预填、空值明确报错、重新
  填写后保存以及侧栏出现项目资源。首次执行在产品断言前因旧隔离运行时未生成新 CDP
  端口而失败；显式重启后原样重跑通过。
- 下一工程动作是发布包含修复的新版本，不再重复扩张本问题的测试范围。
- 新版本在同一真实 V2EX 场景复测成功后，才把本条问题标记为关闭并继续 W1-A 其余真人
  验收步骤。

### 2026-08-18 · 网站账号调整为全局资源库

用户现在能做什么、还不能做什么：

- 用户仍只能使用现有项目账号 v2 实现；全局列表、跨项目同 Profile/Session 和运营矩阵尚未
  实现，不能按新方案验收。
- 事务仍属于当前项目，既有三段式 Tab 和工程组件保留；当时 AI 是否可以调用全局账号尚未
  决定，该历史状态已由 ADR 0016 取代。

本次产品与文档决定：

- “网站与账号”改为全局资源库，一个账号只添加和登录一次，所有项目都能看到并手动打开。
- Browser Tab 仍属于当前工作空间，只引用全局账号和唯一 Profile/Session。
- 事务继续以 `workspaceId` 隔离，可以保存全局账号或运营矩阵引用；引用不等于 AI 授权。
- 增加全局运营矩阵，矩阵只引用账号并通过版本/绑定快照阻止旧事务范围静默扩大。
- AI 调用账号、自动选号、账号并发和自动发布全部保持未决，不进入本轮施工。

文档与工程准备度：

- 新增 ADR 0014，取代 D-09 的单项目账号所有权决定。
- 新增 `global-web-accounts-development-plan.md`，定义 E0、G1–G4、T1、R1 的目标、方案、
  数据生命周期、验收、失败降级和禁止项。
- 产品事实源和架构事实源已同步；历史项目账号记录保留为 v2 迁移证据，不被重新包装成
  新方案验收。

状态与下一步：

- 本次没有代码和用户功能增量；只完成产品决策、架构决定和正式施工计划。
- E0 为 `Ready`；G1–G4、T1、R1 为 `Pending`。下一步只能先做 v3 迁移与回滚门禁，再交付
  G1“添加一次、所有项目可见”的最小纵向闭环。
- A1 需要在全局引用上回归；当时 A2–A4 保持 `Pending`，该历史等待状态已由 ADR 0016 和
  X0–X5 计划取代。

### 2026-08-18 · 全局账号实现进入统一验收

用户现在能做什么、还不能做什么：

- 用户已可以只保存一次网站账号，在所有项目查看并手动打开同一全局账号和 Profile。
- 用户已可以创建版本化运营矩阵，并在项目事务中保存矩阵版本与成员快照。
- 事务仍按 `workspaceId` 隔离，账号全局化没有将事务、材料、流程或证据改成全局对象。
- AI 仍不能自动调用全局账号；UI 显示人工办理边界，主进程返回明确 fail-closed 错误。
- 真实账号身份、2FA 和重启后 Session 仍待真人验收，因此产品里程碑不宣称 `Complete`。

实现与门禁：

- WebResource v3 删除账号 `projectId`，v1/v2 升级时保留 ID、Profile 和登录确认时间，并先备份原文件。
- Browser Tab 改为“全局 `accountId` + 当前 `workspaceRef`”投影，同项目聚焦复用，不同项目不抢占 Tab。
- 重复处理、账号归档、矩阵创建/修改/归档、revision 冲突、事务矩阵快照和 AI 负向门禁均已有定向测试。
- 网站与事务专用 Electron UI smoke 5/5：覆盖跨项目全局账号/Profile、独立 Tab 投影、
  运营矩阵、重启恢复、事务矩阵快照和 AI fail-closed。
...
- E0：`Complete`（工程准备度）。
- G1–G4、T1：`Acceptance`。
- R1：`In Progress`，自动化已收口，仅等待真人真实登录身份、2FA 和重启 Session 复测。
- A2–A4 不作为本轮账号全局化的完成条件；Agent 账号执行后来由 ADR 0016 单独决策和施工。

独立阻塞：全仓 `pnpm smoke:ui` 在前置 CCLink 远程入口用例中出现无退避重连风暴并关闭
renderer，发生在网站与事务用例之前；连续复现后按止损规则转为远程域问题。本轮未修改
远程域，也未宣称全仓 smoke 通过。

### 2026-08-18 · Agent 获得全局账号安全目录读取能力

用户现在能做什么、还不能做什么：

- 用户可以直接询问 Agent 已经在 Studio 登记了哪些网站、账号和运营矩阵；Agent 能读取
  网站名称/域名、账号名称、主体、角色和用户确认的登录状态。
- Agent 仍不能因此打开 Tab、取得 Profile、判断实时网页身份、登录或操作账号。

实现与边界：

- 新增只读 MCP 模块 `web-resources` 和工具 `web_accounts_list`，直接读取
  `WebResourceService` 的全局 Snapshot，不建立第二状态 owner。
- 输出使用专用安全投影，明确排除 `browserProfileId`、`loginHint`、密码、Cookie、Token、
  深层入口 URL、网站备注和网页内容；默认隐藏归档记录。
- 读取工具标记 `readOnlyHint`，不复用 `resolveLaunch`，也不改变既有
  `AI_ACCOUNT_ACCESS_UNDECIDED` 执行门禁。
- 账号目录作为跨域只读事实加入全部 Agent 作用域；浏览器、Android、编辑器作用域原有的
  操作工具边界不扩大。

验收状态：

- G5 进入 `Acceptance`；自动化需验证安全字段和归档过滤，真人需在 Agent 对话中询问一次
  “我注册了哪些网站和账号”，核对回答与“网站与账号”侧栏一致。
- 当时打开、登录、自动选号、网页操作和跨事务执行授权仍是独立产品决定；该状态已由同日
  后续 ADR 0016 和本文第 16 节取代。
- `pnpm verify` 通过：287 个测试文件、1684 项通过、2 项跳过；真实开发实例已注册并成功
  调用 `web_accounts_list`，返回结果未出现 Profile、登录提示、深层入口或秘密字段。该实例
  当前没有登记账号，因此真实内容一致性仍待用户数据验收。
...
## 16. Agent 使用网站账号执行网页任务开发计划

### 16.1 当前范围和完成定义

本节是 ADR 0016 的当前实施事实源。目标不是建设新的自动化平台，而是把四项底层能力连成
真实纵向闭环：读取账号目录、打开指定账号、操作当前网页、人工接管和交还。关联事务时再
把过程与结果写回 `WebAffairService`。

当前代码已经允许 Agent 使用登记账号执行网页任务，但尚未通过 R2 的真实网站真人验收。
只有 X1–X5 全部完成真实验收后，才能声明“Agent 可以接管网页事务”。

首批场景只包括：审核/备案/实名/工商进度查询，驳回或补正原因读取，申请资料和发布信息的
草稿准备，指定材料上传，回执下载，用户接管完成的单次关键动作，以及事务后续检查。运营矩阵
批量执行、后台无人值守提交和平台专用适配器不进入本计划。

### 16.2 架构设计和状态所有权

```text
Agent Thread
  ├─ web_accounts_list：只读安全目录
  └─ web_account_open(accountId)：请求使用明确账号
       ↓
WebResourceService：校验账号并解析唯一入口/Profile，秘密值不返回模型
       ↓
Workspace/Tab：在当前工作空间创建或激活可见 Browser Tab
       ↓
BrowserTask：固定 accountId/tabId/profileId/conversationId/agentRunId 并执行网页动作
       ├─ 普通动作：读取、点击、填写、上传、下载
       └─ 敏感边界：Browser 动作守卫暂停 BrowserTask 并转人工接管
              ↓
WebAffairService：仅在关联事务时保存 Attempt、节点、卡点、证据和等待状态
```
...
| 状态                      | 唯一所有者                         | 生命周期               | 禁止                                |
| ------------------------- | ---------------------------------- | ---------------------- | ----------------------------------- |
| 全局账号和 Profile 绑定   | `WebResourceService`               | 保存到归档/删除        | Agent、事务或 renderer 复制账号事实 |
| Cookie/Session            | `BrowserManager` Profile partition | 跟随全局账号登录环境   | 返回模型或写入资源快照              |
| 当前工作空间 Tab          | Workspace/Tab 基础层               | 打开到关闭，可恢复投影 | 主进程另建隐藏账号 Tab              |
| 单次网页运行和动作日志    | `BrowserTask`                      | 创建到完成/暂停/失败   | 充当跨天事务状态                    |
| Agent 对话和本次意图      | Agent runtime                      | 当前 Thread/Run        | 变成永久账号授权 Store              |
| 事务、Attempt、证据和等待 | `WebAffairService`                 | 事务创建到终态/归档    | BrowserTask 或 UI 自行推断持久状态  |
| 敏感动作阻断              | Browser 工具动作守卫               | 当前动作转人工接管     | Agent 绕过阻断执行最终动作          |

首版同一 `accountId` 只允许一个 Agent BrowserTask 执行。该占用是运行期协调，不持久化成
账号权限；运行结束、失败或应用退出后释放。用户手动接管优先于 Agent，占用冲突时明确暂停
或拒绝，不能并发写同一登录环境。

### 16.3 整体链路逻辑

1. 用户在对话中明确任务和账号，或在事务节点点击“交给 AI”。
2. Agent 读取安全目录；只有一个明确匹配时继续，存在同名、多主体或矩阵歧义时询问用户。
3. Agent 调用统一账号启动 contract；主进程校验账号存在、未归档和运行上下文。
4. renderer 复用现有人工账号启动入口，在当前工作空间创建或激活可见 Tab；不得移动其他
   工作空间的 Tab，也不得创建隐藏页面。
5. BrowserTask 创建时固定账号、Tab、Profile、会话和 AgentRun 关联；关联不完整就失败关闭。
6. Agent 重新观察页面和实际身份。登录失效、身份不明、验证码或风控立即转人工。
7. 普通网页动作连续执行；可能产生最终外部副作用或无法判断风险的动作先暂停确认。
8. 用户接管时 BrowserTask 停止动作；用户交还后 Agent 重新观察 URL、页面和身份再继续。
9. 任务完成必须取得页面状态、申请号、回执、下载文件或其他可复核证据，不能只相信工具
   成功返回。
10. 临时任务在对话中汇报结果并结束；事务任务由 `WebAffairService` 写入 Attempt、节点状态、
    卡点和证据。进入外部等待时结束本次 BrowserTask 和 AgentRun。
11. 到期或用户继续时创建新的运行，重新从显式账号引用恢复现场，不保持后台假运行。
...
明确任务
  ...
  → 结果证据
  ├─ 临时任务：回到 Agent 对话后结束
  └─ 事务任务：写入 Attempt 和节点；等待时关闭运行，后续新建检查运行
```

- 应用退出或 Agent/BrowserTask 崩溃：运行标记中断；账号、Session 和事务事实保留，不能恢复
  为“仍在执行”。
- Tab 被关闭：当前运行失败或暂停；关闭 Tab 不删除账号和事务。
- 登录失效：事务进入“需要我处理”或临时任务提示用户，不自动输入或索取秘密值。
- 人工接管后未交还：保留事务卡点；临时任务正常停止，不后台轮询用户页面。
- 外部等待：本次运行结束；只保存下次检查时间、方法和事务引用，不保存活跃 Page。
- 结果缺少证据：只能保存“待验证/外部状态未知”，不能标记完成。

### 16.5 里程碑 X0：执行契约与安全门禁

- **状态**：Acceptance（工程准备度，不计用户功能进度；真人验收待 R2）
- **目标**：固定唯一账号启动链、运行关联、敏感动作边界和失败降级，为 X1 提供可验证契约。
- **方案**：定义 `web_account_open` 有界工具与 main→renderer 请求/响应；扩展 BrowserTask
  关联 `accountId`；增加同账号单 Agent 运行占用；定义普通动作、敏感动作和未知动作的守卫；
  保留现有执行入口 fail-closed，直到 X1 纵向链完成。
- **验收**：契约 Schema、可信 sender、归档账号拒绝、歧义拒绝、秘密字段负向、跨工作空间
  Tab 不移动、关联缺失 fail-closed、同账号冲突和模块故障降级的自动化全部通过。
- **不得超出**：不得增加权限中心、逐项目配置、全局总开关、平台分支、矩阵批量、后台任务、
  第二 WebResource/Tab Store 或向模型返回 Profile/Session。

### 16.6 里程碑 X1：Agent 打开指定账号

- **状态**：Acceptance；真实账号打开待 R2
- **用户结果**：用户说“打开张三公司的 Apple Developer”，Agent 在当前工作空间打开正确账号
  的可见 Browser Tab；若有歧义则先询问。
- **方案**：Agent 先用 `web_accounts_list` 取得安全候选，再以显式 `accountId` 调用
  `web_account_open`；主进程解析唯一入口/Profile，renderer 复用现有
  `resolveAndOpenWebResourceTab` 语义创建或激活 Tab，随后创建绑定该 Tab 的 BrowserTask。
- **真人验收**：准备同站两个测试账号和两个项目；分别指令 Agent 打开账号，核对实际页面
  身份、同账号复用登录环境、不同账号不串用、项目 B 不移动项目 A 的 Tab；归档和登录失效
  分支给出明确提示。
- **不得超出**：不得自动选择同名账号，不得操作网页，不得读取页面秘密，不得新建隐藏 Tab，
  不得用 Cookie 启发式宣称身份正确。

### 16.7 里程碑 X2：真实网页只读查询

- **状态**：Acceptance；真实状态查询和证据待 R2
- **用户结果**：Agent 能查询审核、备案、实名、工商或应用商店办理状态，读取驳回/补正原因，
  下载允许的回执，并给出可复核证据。
- **方案**：在 X1 的绑定 BrowserTask 上复用现有 Browser MCP 读取和导航能力；执行前重新
  观察页面身份；只读结果关联页面 URL、观察时间、安全摘要、截图或下载文件引用。
- **真人验收**：选择一个真实登录网站执行一次状态查询；Agent 回答必须与页面一致并附证据；
  同站另一个账号不得被读取；退出登录、跨站身份不明和页面结构变化必须停止或转人工。
- **不得超出**：不得填写或提交表单，不得把普通 GET/链接一律视为安全，不得把工具调用成功
  当作业务完成，不得更新无关联事务。

### 16.8 里程碑 X3：通用网页办理准备

- **状态**：Acceptance；真实填写、上传、下载和草稿待 R2
- **用户结果**：Agent 能填写表单、选择和上传指定材料、下载文件并保存草稿，停在最终外部
  动作之前。
- **方案**：复用 Browser MCP 的点击、输入、上传和下载；材料只通过当前任务或事务中的明确
  文件引用提供；主进程动作守卫把提交、发布、发送、删除、协议、付款、安全变更以及无法
  判断风险的动作拦截到 X4。
- **真人验收**：在真实可安全测试的网站完成一次“填写 + 上传测试材料 + 保存草稿 + 下载
  回执/预览”；刷新或重启后草稿仍可核对；故意要求最终提交时必须被阻止。
- **不得超出**：不得读取未授权材料，不得任意浏览工作空间文件，不得默认上传营业执照、
  身份证等敏感原件，不得绕过动作守卫，不得声称已正式提交。

### 16.9 里程碑 X4：人工接管与敏感动作阻断

- **状态**：Acceptance；真实人工交接和敏感动作取消待 R2
- **用户结果**：验证码、扫码、2FA、签名或风控出现时用户能立即接管；交还后 Agent 重新观察
  并继续。最终提交等动作由用户在可见网页中完成，Agent 不代替用户点击。
- **方案**：复用 BrowserTask 暂停/接管/交还和 `PermissionManager`；接管卡展示事务、主体、
  账号、网站、动作、字段摘要、文件和影响，然后把同一可见页面交给用户操作。
- **真人验收**：在真实登录流程完成一次 Agent→人工→Agent 交接；用户在接管页面完成一个
  低风险测试动作，交还后 Agent 正确识别新页面而非沿用旧状态；用户取消时 Agent 不得继续；
  付款、签名和账号安全变更始终保持人工处理。
- **不得超出**：不得新增“始终允许”，不得由 Agent 自己确认或点击最终动作，不得绕过验证码/
  风控，不得在接管期间继续操作，不得用接管卡掩盖批量或不可说明的影响。

### 16.10 里程碑 X5：事务回写、等待和恢复

- **状态**：Acceptance；真实事务等待、重启和恢复待 R2
- **用户结果**：Agent 办理关联事务时，事务 Tab 能看到实际进度、卡点、人工步骤、证据和
  结果；提交后可进入等待，重启或到期后使用同一账号重新检查。
- **方案**：由 `WebAffairService` 创建和结束 Attempt，保存节点状态、接管记录、证据和检查
  计划；BrowserTask/AgentRun 只写自身运行事实。进入等待即结束运行，继续时按事务的显式
  账号引用创建新运行并重新观察。
- **真人验收**：创建一件真实测试事务，依次完成 Agent 查询、人工接管、Agent 继续、保存
  证据、进入等待、重启 Studio、再次检查和节点完成；事务列表和三段式 Tab 的状态一致，
  无证据时不能完成，临时 Agent 任务不会产生假事务。
- **不得超出**：不得让 Agent Thread、Tab、BrowserTask 或 ScheduledTask 拥有事务状态；不得
  等待时保持假运行；不得把事务改成全局对象；不得顺手增加流程设计器、收件箱或平台模板。

### 16.11 统一验收门禁 R2

R2 是验收门禁，不新增用户能力，也不能替代 X1–X5 各自的真人验收。

通过条件：

1. 使用两个本地项目、一个真实网站的两个测试账号和一件项目事务完成 X1–X5 链路。
2. 至少覆盖一次登录失效、一次人工接管、一次敏感动作取消、一次应用重启和一次证据不足。
3. 证明模型和日志中没有密码、Cookie、Token、验证码、恢复码或 `browserProfileId`。
4. 证明普通 Browser、人工打开账号、Editor、Terminal 和其他本地能力在 Agent 执行模块失败
   时仍可用。
5. 受影响 smoke 和 `pnpm verify` 通过；真人步骤、时间、账号、结果和残余风险有记录。

只有以上条件全部满足，X5 和“Agent 可以使用已登记账号办理网页事务”才能标记 Complete。

### 16.12 统一禁止额外设计

X0–X5 施工期间一律不得追加：

- 全局权限中心、复杂权限等级、逐项目账号授权配置或“允许全部 Agent”总开关；
- Apple、阿里云、工商、知识产权、应用商店等平台专用账号启动或流程分支；
- 运营矩阵批量发布、跨账号并发、内容队列、审批流、统计报表和团队权限；
- 后台无人值守提交、云端账号同步、秘密值管理器或 Cookie 编辑器；
- 第二套 Browser/Tab 生命周期、第二 WebResource Store、第二事务状态 owner；
- 用 URL、Cookie、页面标题、最近使用或模型猜测代替显式账号引用；
- 用重构、Schema、Mock、单测、工具数量或 CI 通过宣称用户功能完成。

确有必要改变上述范围时，必须先更新产品事实源和 ADR、说明原闭环为什么无法成立并取得
用户确认；开发中发现“顺手能做”不是扩张理由。

### 16.13 文档修正和维护责任

| 文档                                                    | 本次修正                                              | 后续由谁维护                 |
| ------------------------------------------------------- | ----------------------------------------------------- | ---------------------------- |
| `docs/features/ai-web-affairs-agent.md`                 | 确认四项底层能力、用户心智、敏感边界和产品阶段        | 产品事实变化时先更新         |
| `docs/decisions/0016-agent-web-account-execution.md`    | 固定唯一执行链、状态所有权、失败降级和禁止项          | 架构边界变化时更新或新增 ADR |
| `docs/architecture.md`                                  | 写入 Agent 账号启动、BrowserTask 关联和人工确认不变量 | 实现必须持续符合             |
| `docs/features/global-web-accounts-development-plan.md` | 保留账号资源专项边界，明确执行由 X0–X5 独立施工       | G1–G5/R1 验收时维护          |
| 本文第 16 节                                            | 管理 X0–X5 状态、验收证据、风险和范围变更             | 每次里程碑状态变化时维护     |
| `docs/ops/ai-web-affairs-agent-acceptance.md`           | 区分历史项目账号验收、全局账号验收和新执行链验收      | 真人验收时记录结果           |

历史进度记录保留当时事实，但必须标注已被哪个 ADR 或当前章节取代；不得直接删除历史证据，
也不得让历史“未决”描述继续充当当前产品结论。

### 16.14 2026-08-21 · X0–X5 进入统一验收

- **用户现在能做什么**：Agent 可读取全局账号目录，以显式 `accountId` 在当前项目打开可见
  Browser Tab，使用账号唯一 Profile 执行查询、填写和项目内材料上传/下载；事务节点“交给
  AI”会创建 Attempt 并要求 Agent 先走同一账号启动工具。
- **安全边界**：模型不取得 Profile、Cookie、Token 或密码；Cookie/网络拦截/任意脚本/
  坐标点击等能力在账号任务中硬拒绝；跨入口 Origin、提交、发布、支付、删除、签署和确认
  对话框会暂停任务，必须用户在同一可见页面接管。用户交还时把当前页面 Origin 纳入本次
  任务边界，交还后必须先重新读取页面；该边界不持久化成账号授权。
- **旁路门禁**：Agent 即使从浏览器作用域直接命中一个已登记 Profile，也不能先执行通用
  Browser 工具；必须回到 `web_account_open(accountId)` 建立完整账号关联。
- **生命周期**：同一账号同时只允许一个 Agent BrowserTask；完成、失败或取消后释放；事务
  继续由 `WebAffairService` 保存 Attempt、证据和等待计划，外部等待不保留假运行。
- **工程证据**：新增启动 contract、可信 renderer ack、Tab/Profile 精确绑定、账号任务互斥、
  动作边界和事务安全投影定向测试；68 项定向测试、TypeScript、Lint 和统一 `pnpm verify`
  已通过，包含全量测试与生产构建。
- **Electron smoke**：全量 smoke 在本次范围前置的 CCLink 远程入口检查失败，随后 renderer
  target 崩溃；网站事务专用 smoke 第二次仍在进入网站账号断言前因同一页面/窗口链路崩溃
  退出。主进程日志确认 Browser、WebAffair 和含 2 个工具的 WebResource MCP 模块正常启动，
  但该证据不能替代失败的 UI smoke。按止损规则不继续用重复运行掩盖，R2 保持 In Progress。
- **残余风险**：通用控件文字识别不能证明覆盖所有真实网站；真实登录跳转可能跨 Origin；
  真实站点身份、登录态、验证码、草稿持久性和结果证据只能由 R2 真人验收确认。发现不确定
  页面必须继续 fail-closed，不得为了通过测试扩大允许范围。
