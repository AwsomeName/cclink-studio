...
> 文档版本：2.1
> 基线状态：全局“网站与账号”、运营矩阵和项目事务引用已进入统一验收；AI 是否可以调用
> 全局账号暂未决定，当前 UI 和主进程均 fail-closed。
> 最后更新：2026-08-18
...
按 `workspaceId` 隔离。事务可以保存全局账号或矩阵引用，但 AI 是否可以调用这些账号暂未
决定。本轮不得通过删除 `account.projectId === workspaceId` 校验直接扩大 AI、MCP、
BrowserTask 或定时任务的权限。
...
| 维度         | 当前事实                                                             |
| ------------ | -------------------------------------------------------------------- |
| 用户功能进度 | 全局账号、跨项目手动打开、运营矩阵和项目事务引用已实现               |
| 工程准备度   | v3 迁移、全局 contract、合并别名、矩阵 revision 和 AI 负向门禁已实现 |
| 当前阶段     | E0 `Complete`；G1–G4、T1 `Acceptance`；R1 `In Progress`              |
| 当前阻塞     | 无工程阻塞；AI 账号调用是明确未决项，不阻塞人工账号全局化            |
...
| R1     | 全局账号统一真实 App 验收   | In Progress | G1–G4、T1    | 完成跨项目账号、矩阵、迁移和事务引用闭环 | 同上             |
