...
用户现在还不能使用“个人或公司的网页事务代理人”。当前只有少量预置平台的轻量
运营入口、Browser Profile、Agent、BrowserTask 和人工暂停等工程基础；通用“网站与
账号”、持久事务、动态流程、三段式事务 Tab、人工交还和外部等待尚未实现。

当前项目状态：

| 维度         | 当前事实                                                 |
| ------------ | -------------------------------------------------------- |
| 用户功能进度 | 产品能力未开工；用户尚不能添加任意网站资源或创建持久事务 |
| 工程准备度   | 产品方案、架构边界、开发顺序和验收标准已经形成           |
| 当前阶段     | `E0` Ready，等待进入契约和存储最小冻结                   |
| 当前阻塞     | 无技术阻塞；需要开始第一批实现                           |
| 下一用户增量 | `W1`：任意网站、主体和账号可以保存、登录并在重启后恢复   |
| 完成声明     | 当前只能声明文档基线完成，不能声明任何新产品功能完成     |
...
| ------ | ---------------------- | ------- | ----- | ---------------------------------------- | --------------------------------------------- |
| E0     | 契约和存储冻结         | Ready   | 无    | 无新增用户能力                           | 评审记录、contract/schema 测试                |
| W1     | 网站与账号闭环         | Pending | E0    | 添加任意网站和多主体账号，登录并重启恢复 | `docs/ops/ai-web-affairs-agent-acceptance.md` |
...
| -------- | ---- | -------- | --------------------------------------------- | --------- |
| 工程前置 | E0   | Ready    | 无新增能力；冻结 contract、存储版本和迁移边界 | 1–2 人日  |
| 用户功能 | W1   | Pending  | 任意网站、主体和账号可以保存、登录、重启恢复  | 5–8 人日  |
