...
> 文档版本：2.0
> 基线状态：2026-08-18 产品决定将“网站与账号”调整为全局资源库，事务继续属于项目；
> 现有项目账号代码只作为 v2 迁移基线。AI 是否可以调用全局账号暂未决定，A2–A4 不得因
> 本次改造自动获得新调用范围。
> 最后更新：2026-08-18
...
用户现在可以使用现有“先登录、后保存”流程把账号保存到当前项目，也能使用当前项目事务
和三段式事务 Tab；这些是 v2 代码事实，不再是目标产品边界。用户还不能只添加一次账号后
在所有项目看到同一资源，也不能跨项目复用同一正式 Profile/Session 或创建全局运营矩阵。
...
项目账号的未保存关闭、保存重试、幂等、草稿清理和独立 Profile 工程能力继续复用；
`WebAccount.projectId`、项目级 Snapshot、保存到当前项目文案和跨项目拒绝逻辑进入 v2→v3
迁移范围。旧账号、Profile 和事务引用必须保留，疑似重复记录只能由用户确认合并。

事务已有工程组件保留，但尚未经过真人使用；A1 需要在全局账号引用上重新回归，A2–A4
等待 AI 账号调用产品决定。任何既有单测、smoke 或历史 Acceptance 都不能替代新方案的
跨项目真人验收。
...
| 维度         | 当前事实                                                       |
| ------------ | -------------------------------------------------------------- |
| 用户功能进度 | 只有项目账号 v2 可用；全局账号、跨项目复用和运营矩阵尚未实现   |
| 工程准备度   | v2 草稿/Profile/事务 owner 可复用；ADR 0014 和专项计划已建立   |
| 当前阶段     | E0 `Ready`；G1–G4、T1、R1 `Pending`；A1 需回归，A2–A4 暂停推进 |
| 当前阻塞     | 无工程阻塞；AI 账号调用是明确未决项，不阻塞人工账号全局化      |
| 下一用户增量 | G1：添加一次账号后切换项目仍看到同一账号                       |
| 完成声明     | 只能声明文档和工程准备度；不能声明全局账号或事务闭环完成       |
...
| ------ | --------------------------- | ------- | ------------ | ---------------------------------------- | ---------------- |
| E0     | 架构与迁移门禁              | Ready   | 无           | 无新增用户能力；v2 数据具备安全迁移条件  | 全局账号专项计划 |
| G1     | 全局账号列表与一次添加      | Pending | E0           | 一个账号只添加一次，所有项目都能看到     | 同上             |
| G2     | 跨项目 Tab 与登录态复用     | Pending | G1           | 各项目复用同一 Profile/Session           | 同上             |
| G3     | 旧账号迁移、疑似重复与归档  | Pending | G2           | 升级不丢账号、登录态和事务引用           | 同上             |
| G4     | 全局运营矩阵                | Pending | G3           | 一次配置账号集合，所有项目复用           | 同上             |
| T1     | 项目事务引用兼容            | Pending | G3、G4       | 事务仍属项目并引用全局账号/矩阵          | 同上             |
| R1     | 全局账号统一真实 App 验收   | Pending | G1–G4、T1    | 完成跨项目账号、矩阵、迁移和事务引用闭环 | 同上             |
