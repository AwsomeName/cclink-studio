...
> 状态：角色中心 V1 已实现并进入异机验收；七个内置角色的 Manifest / `SOUL.md` 内容
> 已形成 R1 可验收候选；真实模型行为与异机重启尚未验收，版本化 Skill 集成尚未完成。
>
> 最后更新：2026-08-13。
>
...
- 当前受影响工程证据：6 个测试文件 119 项测试、TypeScript typecheck、ESLint、production
  build 和整套 `pnpm smoke:ui` 通过；UI smoke 逐个验证七个角色的 `SOUL.md`、概览和示例，
  并继续验证单例配置页与显式 Skill 挂载。正式发版仍以目标 SHA 的远端普通 CI 和发布工作流
  为事实源。
- 尚未完成的产品证据：打包后在另一台机器执行“重启后第三次发送”与七个角色行为样例
  的真人验收。因此 V1 和 R1 只能称为“可验收候选”，不能称为产品完成；Skill 当前仍是
  可见浅集成，缺少版本化会话引用、立即持久化和运行回执指纹，必须由 R2 收口。
...
执行顺序固定为 R0 -> R1 -> R2 -> R3 -> R4。R0 的异机真人验收等待用户执行时，可以继续
交付 R1 的用户可见内容，但不得把 R0 标记完成。R2 完成前不启动 R3，避免把当前仅传名称和
描述的 Skill 浅集成固化进自定义角色持久化模型。
...
下一步最该做的是并行关闭两个明确缺口：真人按 R0 / R1 验收“同一会话连续两次发送、切换
角色不分叉、重启后第三次发送”和七个角色行为样例；工程主线进入 R2，把当前只传名称与描述
的 Skill 浅集成改为版本化引用、主进程解析、立即持久化和逐轮回执。两项通过前，不得只凭
UI 内容和自动化宣布角色系统完成，也不得启动自定义角色持久化。
