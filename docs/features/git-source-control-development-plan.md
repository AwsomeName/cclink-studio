...
- 检测部分 staged 文件，默认只提交 staged 部分；不得默认全选或在没有用户点击时暗中扩张。
...
- 可提交完整文件分组提供一键全选/取消全选，并显示已选数量；批量选择仍复用同一组精确路径，
  不新增 `git add --all` 捷径。
- 提交草稿按 workspace/revision 隔离；Esc/关闭按钮直接关闭并清理草稿，写操作执行中禁止关闭。
...
- 提交信息和文件选择由 Git store 按工作空间保存；背景点击不关闭，Esc/关闭按钮直接关闭并
  清空本次草稿，不追加二次确认；写操作进行中禁止关闭。
- 刷新后 revision 变化会停止写操作，用户必须接受最新状态；切换工作空间会清空旧窗口和
  草稿，不能把旧项目选择投射到新项目。
- Electron smoke 已验证 Esc 直接关闭、重新打开后草稿为空、单独提交、推送已有提交和“提交
  并推送”两条写路径，并先验证全选/取消全选再只勾选单文件；未选择的 untracked 文件始终保留。
