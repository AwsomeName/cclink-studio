...
CCLink Studio 必须支持**不登录即可进入本地工作台**。

登录不应该是打开 App 的门槛，而应该是解锁官方账号、设备网络、消息网络、跨设备同步和远程配对的能力开关。用户第一次打开 CCLink Studio 时，系统应自动生成一个稳定的本地身份 ID，并用这个身份保存和恢复本机工作现场。

拷问：如果用户只是想写一篇 Markdown、打开一个本地项目、用本地 Terminal 或恢复昨天的标签页，却被手机号登录挡住，CCLink Studio 就不像桌面工作台，更像云 SaaS 壳。这会伤害最基础的可用性。
...
不要把本地身份叫“游客”。游客暗示临时、可丢；CCLink Studio 的本地身份应该是长期本机身份。
...
- 相关提示文案不把本地用户称为“游客”。
- 当前实现：账户区区分“本机身份”和“CCLink 账号”；CCLink 未登录态提示登录并禁用云身份操作。
...
- 如果登出清本地状态，用户会把 CCLink Studio 当成不可信的本地工具。
