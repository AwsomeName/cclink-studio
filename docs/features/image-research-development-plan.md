...
> 状态：2026-09-06 已完成 M0 的登录、搜索、详情和重开能力取证，并完成 R1 的真实“候选—重启恢复—跳过—继续”部分。平台裸 `noteId` 重开已证实失败并完成持久降级；用户亲自保存确认、达标停止和最终统计仍待验收，当前不能宣称 V0 完整交付。
...
- 登录态真实搜索已返回 10 条有界结果，Agent 已打开两条真实笔记并逐轮提出候选；等待候选时重启和跳过后唯一 continuation 已通过。
- 裸 `/explore/{noteId}` 重开真实返回平台 404，不能作为稳定来源；`sourceRecoveryStatus=unavailable`、持久警告和两个仍可用的决定按钮已在真实应用验证。
- 小红书独立 Profile 在启动后创建时不依赖通用 Playwright Page 投影；图片调研使用 main-owned 固定可见页脚本，并按 Tab runtime generation 与 WebContents ID 防止旧 View 串入。该脚本能力没有进入 preload、IPC 或 Agent 工具面。
...
| 阶段 | 性质           | 结果                                               | 工程重点                                 | 预计   |
| ---- | -------------- | -------------------------------------------------- | ---------------------------------------- | ------ |
| M0   | 平台预检       | 已验证登录、搜索、详情；裸 noteId 重开失败并降级   | 登录结果可读，重开进入平台 404           | 已完成 |
...
| R1   | 交付验收       | 真实候选、重启、跳过继续已过；保存达标仍待用户操作 | 2026-09-06 本轮 `pnpm verify` 已通过     | 进行中 |

工程实现和账号前提已经形成；剩余交付只取决于用户完成一次真实手工保存确认，以及本轮最终自动化门禁结果。

### 2026-09-06 真人记录

- 已保存账号的 Profile 保持登录，无登录遮罩；搜索“奥森拍照”返回 10 条有界文字结果。
- Agent 依据文字选择一条结果，打开详情，取得 `noteId=6a26934200000000170095d6`、`imageIndex=0`，持久化候选后停止，BrowserTask 已结束。
- 候选等待时重启应用，仍恢复为同一 Attempt 的 `waiting-human`，没有假运行；用户决定按钮保持可用。
- 以裸 `/explore/{noteId}` 重开时平台返回 404。系统将失败持久化为“来源不可恢复”，不把 404 当作原候选，也不禁用两个决定。
- 点击“跳过并继续”后只把同一 Attempt 从 generation 8 增至 9，且 `decisionOperationId` 等于新 `launchOperationId`；Agent 随后提出第二张候选并再次停止。
- 待验收：用户亲自保存第二张候选并点击“我已自行保存并继续”，确认目标 1 达标后不再启动浏览器动作，统计为保存 1、跳过 1、待处理 0。
