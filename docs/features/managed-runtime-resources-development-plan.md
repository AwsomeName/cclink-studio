...
> 状态：R1/R4 已完成，R2 待真机验收，R3 已完成制品准备但尚不能激活；最后更新：2026-08-12。
...
- 退出 App A、完整替换 `.app` 后启动 App B，三项安装记录逐字节不变且 UI 不要求重装：通过。
...
- 通用远程签名目录、卸载、垃圾回收、真实双版本回滚：未实现。
- arm64 本地 DMG/ZIP 打包与 packaged App A → App B 覆盖替换 smoke：通过。
