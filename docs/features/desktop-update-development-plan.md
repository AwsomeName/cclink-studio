...
> 状态：计划已冻结；U0 已完成并关闭，R0 干净双架构真人安装验收待完成。U1 可用
> 纵向链路已实现但尚未关闭；U2 下载与校验主链已实现，恢复和真实公开版本验收待完成；
> U3-U5 尚未开始。
...
R0 和 U0 是工程前置条件；U1-U5 才是用户功能里程碑。桌面更新不以一个大 PR
同时完成，但拆分不能把“基础设施完成”包装成“产品有进展”。每个用户功能里程碑
必须关闭自己的状态、生命周期、失败恢复和真实应用验收，才能计入产品进度：

| 类别     | 里程碑 | 交付结果                             | 当前状态                         | 可对外描述                   |
| -------- | ------ | ------------------------------------ | -------------------------------- | ---------------------------- |
| 工程前置 | R0     | 双架构签名、公证、Draft Release      | 自动化通过，真人验收待补         | 仅报告工程准备度             |
| 工程前置 | U0     | Manifest、共享契约和发布元数据       | 已完成                           | 仅报告工程准备度             |
| 用户功能 | U1     | 检查服务、状态机和更新界面           | 主链完成，设置/诊断/真人新版待补 | 已支持 Studio 内手动检查     |
| 用户功能 | U2     | 应用内可靠下载、校验和恢复           | 下载校验完成，恢复/真人验收待补  | 下载引擎已接通，尚未关闭     |
| 用户功能 | U3     | 已校验 DMG 的辅助安装闭环            | 未开始                           | 尚不能安装                   |
| 用户功能 | U4     | 用户确认后的自动安装、工作保护和重启 | 未开始                           | 尚不能自动替换并重启         |
| 用户功能 | U5     | 两轮真实发布稳定化、诊断和运营门禁   | 未开始                           | 尚未达到长期维护基线         |
...
- 开始一个阶段前，先写出用户在真实 Studio 中执行的验收动作；无法写出时，该阶段只能
  归类为工程前置，不能计入产品进度。
- 优先实现最小纵向闭环。U1 的第一批必须让用户在 Studio 内主动检查并看到“已是最新、
  发现新版或检查失败”之一，不能先连续多小时扩展发布流水线。
- 单项工程前置工作时间盒为 60 分钟；超时必须停止并汇报阻塞、已经获得的用户能力、
  替代路径和继续投入的理由。
- 同一阻塞连续失败两次必须触发止损，不得在没有用户确认的情况下继续第三轮同类
  尝试。
- 持续任务每 60 分钟执行一次偏航检查，固定回答：过去一小时用户新增了什么能力、
  当前还不能做什么、下一小时交付哪个可见结果。
- mock、测试、CI、Draft、签名或文档通过只记录为工程证据；没有真实 Studio 验收时，
  用户功能档位不得提高。
...
8. 用户功能里程碑必须附真实 Studio 中可执行的验收步骤和结果；工程前置不得冒用
   本条声明产品能力完成。
...
进度分成两条，禁止合并成一个容易误导的数字：

1. **用户功能进度**：只计算 U1-U5，默认对用户汇报这一项。
2. **工程准备度**：分项报告 R0、U0 和各项门禁，不与用户功能进度相加。

用户功能进度按真实可验收结果计算：

| 用户结果                         | 对应里程碑 | 权重 |
| -------------------------------- | ---------- | ---- |
| 在 Studio 内发现版本并明确提示   | U1         | 25%  |
| 在 Studio 内下载、取消、重试校验 | U2         | 25%  |
| 打开可信安装包并完成辅助安装     | U3         | 15%  |
| 用户确认后自动安装并重启         | U4         | 25%  |
| 两轮真实发布稳定化               | U5         | 10%  |
...
- **用户功能进度：`0%`。** 当前 Studio 不能检查、下载或安装更新。
- **工程准备度：** R0 自动化完成但双架构干净机器真人验收未完成；U0 contract、
  Manifest、真实 Draft、独立验证和失败拦截均已完成。
- U1-U5 均未开始。下一项只认 U1 在真实 Studio 中完成检查并展示结果，不再以发布
  流水线、测试数量或内部提交提高用户功能进度。
...
历史第一轮只收口了工程前置 U0，没有实现 UI、下载或安装；该轮不计入用户功能
进度：
