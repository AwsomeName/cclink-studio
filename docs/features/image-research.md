...
> 状态：2026-09-06 已完成真实登录账号下的搜索、打开笔记、提出候选、等待时重启恢复、跳过并继续一轮。真实页面证明裸 `noteId` 重开会进入平台 404，因此已实现并验证“来源不可恢复”持久降级；用户亲自保存确认、达标终态及其最终统计仍待完成，当前不宣称完整交付。
...
- Agent 运行只获得四个图片调研工具，且关闭 builtin tools；main-owned 小红书可见页适配器只返回标题、作者、有限可见文字和短期引用。独立账号 Profile 不依赖通用 Playwright Page 投影，固定脚本只在 main 内部按 Tab runtime/WebContents 身份执行，不通过 IPC 暴露任意 evaluate。
- Store 已改为接收完整 `changedAffairIds`；图片或混合批次不再生成“文章事务子集 + 全局 revision”的恢复 journal。
- 2026-09-06 登录态真人实测：已保存账号 Profile 可搜索，Agent 得到 10 条有界结果、打开真实笔记并提交一张候选后停止；等待决定时重启，候选、单 Attempt 和 generation 均正确恢复；跳过后只启动一个新 generation，并再次得到候选。
- 同日实测确认：将已观察 `noteId` 构造为 `/explore/{noteId}` 后，平台返回“当前笔记暂时无法浏览”的 404，说明裸路径不是稳定恢复能力。系统现会持久化 `sourceRecoveryStatus=unavailable` 和原因，并保持“我已自行保存”“跳过并继续”可用，不声称来源已恢复。
- 完整交付前仍需用户对当前真实候选亲自保存并确认，验证达标后停止和最终统计；登录失效接管、列表刷新旧引用、同账号成功重开期间的租约互斥仍保留为专项真人回归项。
