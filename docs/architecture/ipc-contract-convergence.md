...
CCLink Studio 已经有共享 IPC contract 基础设施，但当前覆盖不完整。Terminal、Android、
数据源以及若干较新的能力仍在 main 与 preload 中分别手写同一个通道字符串；共享文件只约束
TypeScript API 形状，没有成为通道名、参数解析、错误语义和事件生命周期的唯一事实源。
...
本项工作不增加新按钮或新能力。完成后，用户应能在真实 Studio 中继续完成以下原有动作，且
行为与改造前一致：
...
2. 触发一条需要确认的 Terminal 命令，确认请求只出现在所属工作空间，批准与拒绝均能返回
   与改造前相同的错误类型、错误码和关键提示；生命周期操作后 runtime、权限策略和关闭策略不丢失。
...
4. 在已配置只读测试数据源的环境中列出数据源、测试连接、列出集合、执行查询、保存并重新打开
   Saved Query；非法输入仍返回改造前相同的结构化错误。复制诊断并检查 workspace 持久化结果，
   password、token、apiKey 和完整凭证不得出现。
...
目前两边各自手写字符串。`TerminalApiContract` 只能检查 preload 对 renderer 暴露的方法形状，不能
证明 main 注册了相同 channel，也不能让 `ipcRenderer.invoke()` 在编译期拒绝错误的字符串或参数。
...
2026-08-28 对生产源码的只读库存如下。计数只覆盖 preload 中直接使用字面量的 IPC，不代表整个
仓库的 IPC 总数。
...
- 在进入 P1 前处理 Editor 三处 `removeAllListeners`：为 content update、read request、save request
  增加并存订阅与精确释放测试；每次订阅只移除自身 handler。如果产品语义要求单例，只能保存并
  替换该 API 自己拥有的 handler，不能清空整个 channel 的其他所有者。

退出标准：库存中的每个通道有 owner 和处置结论；现有用户行为由测试固定。P0 只是工程准备度，
不能声明用户能力新增或迁移完成。
...
`registerTerminalIpc` 当前忽略了 `terminalExecutionAdapter.onEvent()` 返回的 disposer，IPC
registration scope 也不会自动拥有这个领域事件订阅。P1 的最小修复是让 `registerTerminalIpc`
返回该 unsubscribe（没有 adapter 时返回 no-op），由 runtime 保存为
`terminalExecutionEventUnsubscribe`；Terminal reset、窗口/服务重建失败回滚和主进程 shutdown 必须
在清空 adapter 引用前调用并置空。测试覆盖重复注册、单次 dispose、重复 dispose 和重建后只有一个
execution event 投递。
...
不丢失；逐通道 rejection/结构化失败、错误码和关键错误文本与基线一致；真实 Terminal 验收通过；
...
退出标准：7 个 invoke 同源；真实只读数据源的连接、查询和 Saved Query 验收通过；逐通道非法参数、
服务不可用错误码和关键错误文本与基线一致；创建、测试、查询、复制诊断和工作空间持久化均证明
password、token、apiKey 及完整 credential 不进入 renderer payload、日志、诊断或 workspace。
数据源 namespace 随本阶段退出 P0 allowlist。
...
| 安全回归         | Terminal 精确错误兼容；数据源凭证不进入 renderer、日志、诊断或 workspace                |
...
- 如果 parser 前移改变了现有错误类型，先恢复兼容语义，再继续迁移；不要要求 renderer 同时大改。
