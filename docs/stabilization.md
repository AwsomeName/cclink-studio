...
> 2026-07-28 说明：本文保留 S1 当时采用 `safeStorage` 的历史验收事实。凭证目标架构已由 ADR 0003 改为 OSS 本地明文存储，实施进度见 `docs/features/local-credentials-development-plan.md`。

## 结论

S0-S4 已完成，稳定化阶段已于 2026-07-21 关闭。当前允许从 `main` 稳定基线受控恢复功能开发；任何新功能仍必须遵守架构宪法、模块边界和质量门禁，不能重新引入本阶段已经关闭的问题。

本文保留稳定化的工作包、证据和退出标准，作为后续回归判断的历史事实源。
...
稳定化期间新增业务功能默认延期；该限制随 S4 关闭解除。后续功能仍需在设计中说明权限面、跨模块依赖、验证和回滚边界。
...
S4 库存与分包见 `docs/ops/stabilization-s4-state-inventory.md`。S4.1 已关闭：Terminal 主进程 registry/store 是 session 事实边界，Terminal Tab 降为可见投影；项目 hydrate 后从 `terminal:listSessions` 对账，过期 workspace 结果被丢弃，查询失败不阻断项目切换。实现提交 `7b9f81e` 在当前工作树与全新 detached worktree 均通过 141 个测试文件/849 项测试、standalone 24/24 与严格认证 smoke，GitHub Actions run `29816406350` 成功。

S4.2 已关闭：workspace transition 在 Browser/Terminal 异步准备完成和 generation 复核后，才同步提交文件树、workspace identity 与各运行时投影；选择器、最近项目和关闭项目共用互斥状态，失败或过期切换不再留下半提交现场，空快照也不会继承上一项目的文件树路径。`workspace-store` 只保留一个身份提交入口，最近项目与文件树准备职责从 `fs-store` 拆出，启动恢复补做 Terminal 事实对账。实现提交 `e08150d` 在当前工作树与全新 detached worktree 均通过 143 个测试文件/858 项测试、standalone 24/24 与严格认证 smoke，GitHub Actions run `29823522729` 成功。S4 下一工作包为 S4.3 conversation/browser profile 投影与高变模块拆分；S4 与稳定化阶段继续。
...
- 第三方凭证遵守 ADR 0003：只由主进程 `CredentialService` 写入独立、受限权限的本地明文文件，不进入工作空间、普通设置、日志、诊断或 renderer 全量状态。
- IPC 生命周期与参数校验具有单一声明源，不再依赖人工维护清理清单。
- 登录状态、工作空间切换、窗口重建和任务状态可从诊断日志明确定位。
