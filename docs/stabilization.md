...
当前进度：S1.1-S1.4a 已完成，库存、失败边界和证据见 `docs/ops/stabilization-s1-security-inventory.md`。S1.1 关闭微信公众号 HTML 直注入并启用主 renderer sandbox；S1.2 将 Agent/Meshy 密钥迁移到 `safeStorage`，公共设置与 renderer 不再读取原文；S1.3 为主 renderer 增加开发/生产分离的 CSP，并为窗口、设置、文件和 Terminal IPC 建立统一 trusted sender guard；S1.4a 将相同 guard 和有界 schema 扩展到 Browser、Android 与数据源，撤销 renderer Meshy 及未使用的 raw shell/写入入口，并按能力拆分首批 preload。S1 尚未完成，S1.4b 仍需覆盖 Agent、项目运营、Git 备份、CAD/硬件、WorkspaceState 等剩余 IPC 和资源作用域，不得恢复功能扩张。
