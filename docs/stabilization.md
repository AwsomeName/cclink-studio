...
当前进度：S1.1 与 S1.2 已完成，库存、失败边界和证据见 `docs/ops/stabilization-s1-security-inventory.md`。S1.1 关闭微信公众号 HTML 直注入并启用主 renderer sandbox；S1.2 将 Agent/Meshy 密钥迁移到 `safeStorage`，公共设置与 renderer 不再读取原文。S1 尚未完成，下一包处理主 renderer CSP、高权限 IPC sender 和运行时 schema，不得恢复功能扩张。
