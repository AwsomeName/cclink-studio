...
S3.2b1 进行中：shared `IpcInvokeContract` 已建立，main 注册器自动执行参数 parser，preload client 使用同一 channel/result contract；Window、Identity、Official 为首批迁移域。带参数的高权限 Settings、Dialog、FS、Agent 与 Browser 仍待后续批次迁移，S3.2 尚未关闭。
