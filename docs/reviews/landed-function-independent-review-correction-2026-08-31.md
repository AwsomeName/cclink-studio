...
| RF-01 | P0           | 核心工程实现完成；真实 App 验收与 TOCTOU residual 待结论 | 文件权限边界不统一                                   | `FileService` 已收敛到主进程 active/trusted workspace；WorkspaceState 绝对路径 IPC 也要求 active/picker capability | Node 无可移植 `openat`，递归 mkdir/rename/copy 的父目录瞬时替换仍需按 residual threat 管理；真实文件选择器与 App 回归待统一验收 |
