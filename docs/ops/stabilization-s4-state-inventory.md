...
> 状态：S4.1-S4.4 已关闭，稳定化阶段完成。分支：`codex/stabilization-s4`。起始基线：`1059ba6`。S4.1 实现基线：`7b9f81e`。S4.2 实现基线：`e08150d`。S4.3a 实现基线：`f29e51d`。S4.3b 实现基线：`4cdd1d5`。S4.4 修复基线：`f07dbea`。日期：2026-07-21。
...
- Agent 日志分别拥有 workspace、conversation、run、backend session、BrowserTask 和 Profile 信息，但 BrowserTask 只保存 `tabId`；这些值同时出现不等于能证明属于同一次操作。
- 复制诊断按 Tab 选择最新任务，同一 Tab 被多个会话复用时可能拿到其他会话的任务并误报归因。
- 原始 Session ID 属于敏感凭证，不能为了关联而直接复制、截断或生成长期稳定的非密钥哈希。
- Browser action 时间线已有 `taskRunId` 字段，但文本输出没有显示，故障动作仍需人工从不同区段猜测归属。

### 收敛方案

- `BrowserTaskRun` 增加兼容旧记录的可选 correlation 块，由 AgentBridge 在任务创建时写入 workspace、conversation、run、Profile 和主进程随机 Session 引用；stream/complete/error/abort 时补齐后端实际 run/session，并只在变化时发布任务事件。
- Session 引用由主进程按进程生命周期随机分配；重置、关闭会话和销毁 runtime 时释放。它不包含原始 Session ID，也不是可跨重启追踪的凭证派生值。
- 诊断任务选择先匹配当前 workspace + conversation + tab；只对无 correlation 的历史任务做兼容回退，绝不回退到另一个已知会话的任务。
- 复制诊断新增 `关联链`，逐项比较 workspace/conversation/run/session/tab/profile，输出 `matched`、`incomplete` 或 `mismatch` 以及缺失/错配字段；Browser action 起止事件显式携带 `taskRunId`。
- 脱敏处理改为幂等，敏感 URL 参数使用占位符后再清洗，避免二次处理破坏 `[redacted]` 输出。

### 自动化验收

- [x] BrowserTask correlation 使用深拷贝，外部不能修改 runtime 事实；相同 patch 不重复发事件。
- [x] 随机 Session 引用在进程内稳定、不同 Session 不同、删除/clear 后轮换，且不包含原始 Session ID。
