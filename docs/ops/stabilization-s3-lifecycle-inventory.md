...
S3.2b 继续把 main/preload 的通道名和运行时 schema 收敛到共享 contract。S3.2b1 已建立 `IpcInvokeContract<Args, Result>`：shared 声明携带 channel、参数解析器和结果类型；main registrar 在调用 handler 前自动解析，preload client 从同一 contract 调用。首批 Window、Identity、Official 无参数高权限通道已迁移，并由源码边界测试禁止退回 main/preload 双写字符串。

下一批需迁移带参数的高权限 Settings、Dialog、FS、Agent 与 Browser contract；完成前，其他通道字符串仍可能在调用端漂移，S3.2 和整个 S3 都不能关闭。
