...
- 禁止把局部安全修复、诊断增强、测试补强或前置工程当成用户要求的主线交付。安全修复必须保留，但只能报告其实际解决的问题，不能替代主线验收，也不能通过降低保护条件或关闭原功能完成主线。
- 开始实现前必须明确本轮用户要验收的能力、当前缺口和完成标准；每项改动必须对应主线验收项或有证据的直接阻塞。发现支线工作正在替代主线时立即停止扩张，说明尚未交付的能力和回到主线的下一步，不得等到用户追问才承认偏航。

### 文章发布执行计划的交付红线

- “细化执行计划”必须落在用户看到的执行计划和真实执行链路上，覆盖准备、账号与原稿核验、正文与图片、平台字段、保存、提交发布及结果核验；只增加文案、静态子步骤或恢复／Runtime／首次检查的局部动作，不算交付。
- 每个可执行小步骤必须明确：执行者（Studio／Agent／用户）、要做的动作、进入条件、当前实际状态、完成条件与核验证据、失败或等待原因、允许的下一步。粒度以能独立判断“是否开始、是否完成、为何卡住”为准，不以凑步骤数量为准。
- WebAffair 是唯一发布进度状态所有者；细步骤状态必须由真实执行事件和核验结果驱动，UI 只呈现事实，不得按计时、Agent 自述或前端推测标记成功。复用现有 current operation、transition、长 Agent Run 和 Runtime handshake，不借此新增完整 operation 账本、claim/report MCP、短 Run 调度或通用框架。
- 执行计划可观测性与提交发布闭环必须分别验收：前者必须在真实应用中看到小步骤随执行、等待、失败、中断恢复而变化；后者必须完成实际提交及平台结果核验。首次 inspect 成功、保存草稿成功或自动测试通过，均不能宣称文章发布闭环完成；未获具体账号和文章的公开发布授权时不得擅自发布，也不得把未验收写成已完成。
...
- 正式发版脚本持有 `.git/cclink-release.lock` 期间，其他任务不得在该仓库提交或推送
  `main`；必须等发布结束释放锁。脚本还会在版本提交前后校验本地与远端源 SHA，发现漂移
  立即停止，禁止把未经过匹配 CI 的并行提交混入 Release。
- 正式 Release 在全部自动门禁通过后直接公开，真实用户安装与更新反馈属于公开后的测试；
  不再设置发布前真人安装确认或 Draft 转正步骤。自动门禁失败时仍禁止公开半成品。
