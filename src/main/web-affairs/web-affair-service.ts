...
import type {
  ...
  ArticlePublishingOperationTransition,
...
const ARTICLE_OPERATION_TRANSITION_LIMIT = 200
...
export interface ArticlePublishingAgentReporter {
  ...
  trustedPageEvidence?: {
    ...
    /** Main adapter compared the complete frozen body, links and image positions. */
    bodyMatchesFrozen?: boolean
  }
}

export interface ArticlePublishingRecoveryVerification {
...
export class WebAffairService {
  ...
  createArticlePublishingAffair(
    input: {
      ...
      composer?: import('../../shared/article-publishing/article-publishing-types').ArticlePublishingComposer
  ...
  startArticlePublishingFirstInspect(input: {
    workspaceId: string
    affairId: string
    attemptId: string
    executionGeneration: number
    launchOperationId: string
    expectedOperationRunId: string
    expectedOperationRevision: number
    runtime: ArticlePublishingRuntimeSnapshot
  }) {
    return this.enqueueScoped(input.affairId, input.workspaceId, () =>
      this.transitionArticlePublishingFirstInspectNow(input, false),
    )
  }
  ...
  completeArticlePublishingFirstInspect(
    ...
    return this.enqueueScoped(input.affairId, input.workspaceId, async () =>
      isObservationCurrent && !isObservationCurrent()
        ? this.evidenceRequired('首次检查证据在进入状态事务前已过期')
        : this.transitionArticlePublishingFirstInspectNow(input, true),
    )
  }

  failArticlePublishingCurrentOperation(input: {
    workspaceId: string
    affairId: string
    attemptId: string
    executionGeneration: number
    ...
    return this.enqueueScoped(input.affairId, input.workspaceId, () =>
      this.failArticlePublishingCurrentOperationNow(input),
    )
  }
  ...
  recordArticlePublishingPlanResults(
    input: {
      ...
      executionGeneration: number
      launchOperationId: string
      results: Array<Pick<ArticlePublishingDetailResult, 'id' | 'status' | 'evidence' | 'reason'>>
    },
    isCurrent: () => boolean,
  ) {
    return this.enqueueScoped(input.affairId, input.workspaceId, async () => {
      ...
      if (
        ...
        !['preparing', 'running'].includes(publishing.execution.status)
      )
        return this.transitionError('细步骤证据已失效或执行已停止')
      let next = publishing
      const now = this.timestamp()
  ...
  /** Internal observer only. Bind the one-file upload to its unique new, loaded editor image. */
  recordArticlePublishingImageObservation(
    input: {
      workspaceId: string
      affairId: string
      attemptId: string
    ...
    return this.enqueueScoped(input.affairId, input.workspaceId, async () => {
      ...
      return this.persistAffair({
        ...
        articlePublishing: {
          ...publishing,
          assets: publishing.assets.map((a) =>
            a.id === asset.id ? { ...a, platformUrl: input.platformUrl } : a,
          ),
        },
      })
    })
  ...
  recordWeiboSubmissionReceipt(
    ...
    return this.enqueueScoped(input.affairId, workspaceId, async () => {
      ...
      if (
        ...
        effect.browserTaskRunId !== input.browserTaskRunId ||
        !['dispatched', 'result-unknown', 'verified'].includes(effect.status) ||
        !parseWeiboPublicationUrl(`https://weibo.com/${input.uid}/${input.postId}`)
      )
        return this.transitionError('微博提交回执不属于本次文章及已派发发布动作')
  ...
  recordArticlePublishingPageObservation(
    ...
    return this.enqueueScoped(input.affairId, workspaceId, async () =>
      isObservationCurrent && !isObservationCurrent()
        ? this.evidenceRequired('保存读回证据在进入状态事务前已过期')
  ...
  dispatchArticlePublishingSideEffect(
    affairId: string,
    attemptId: string,
    executionGeneration: number,
    sideEffectKey: string,
    ...
    return this.enqueueScoped(affairId, workspaceId, () =>
      this.dispatchArticlePublishingSideEffectNow(
        affairId,
        attemptId,
        executionGeneration,
        sideEffectKey,
  ...
  private async createArticlePublishingAffairNow(
    ...
    const original = input.reviseDraftFromAffairId
      ? this.findAffair(input.reviseDraftFromAffairId)
      : undefined
    const originalPublishing = original?.articlePublishing
    if (input.reviseDraftFromAffairId) {
      if (
        !original ||
        original.workspaceId !== workspaceId ||
        !originalPublishing ||
        !['cancelled', 'failed'].includes(originalPublishing.execution.status) ||
        originalPublishing.accountId !== input.accountId ||
        originalPublishing.fields.title !== input.fields.title ||
        !originalPublishing.draft?.platformDraftId ||
        !originalPublishing.draft.platformAccountId ||
        originalPublishing.publication.status !== 'not-started' ||
        originalPublishing.sideEffects.some(
          (effect) =>
            effect.kind === 'publish' ||
        ...
        return this.invalid(
          '只能修订同工作空间、同账号同标题、已终止且没有未知动作或发布记录的原草稿',
        )
      }
      if (
        this.snapshot.affairs.some(
          (candidate) =>
            ...
            candidate.articlePublishing?.accountId === input.accountId &&
            candidate.articlePublishing.draft?.platformDraftId ===
              originalPublishing.draft?.platformDraftId &&
            !['cancelled', 'failed'].includes(candidate.articlePublishing.execution.status),
        )
      ) {
        return this.invalid('此原稿已有修订任务，请继续现有任务，禁止重复创建')
      }
    }
    ...
    const articlePublishing = articlePublishingStateSchema.parse({
      ...
      ...(originalPublishing?.draft
        ? {
            draft: {
              platformDraftId: originalPublishing.draft.platformDraftId,
              platformAccountId: originalPublishing.draft.platformAccountId,
              url: originalPublishing.draft.url,
              normalizedTitle: originalPublishing.draft.normalizedTitle,
            },
          }
        : {}),
  ...
  private async startAttemptNow(
    ...
    const publishing = affair.articlePublishing
    const recoveryOperation = publishing?.draft?.platformDraftId
      ? this.createArticlePublishingOperation(
          ...
          now,
          '从管理页核验待修订原稿',
          '原账号、draftId、标题与 saved 全部匹配后开始修订',
        )
      : undefined
    return this.persistAffair({
      ...affair,
      ...
      ...(affair.articlePublishing
        ? {
            articlePublishing: {
              ...affair.articlePublishing,
              ...(publishing && recoveryOperation
                ? {
                    draft: this.beginArticlePublishingRecovery(
                      publishing,
                    ...
                    executionProtocol: {
                      ...this.appendArticlePublishingTransition(publishing, {
                        operationRunId: recoveryOperation.operationRunId,
                        kind: 'recovery-started',
                        occurredAt: now,
                        summary: '修订任务从管理页核验同一原稿；不沿用旧内容完成记录',
                      }),
  ...
  private async finishAttemptNow(
    ...
    if (found.affair.articlePublishing) {
      ...
      if (
        ...
        found.affair.articlePublishing.executionProtocol.current
      ) {
        return this.transitionError('当前 operation 尚未完成，不能把文章发布事务直接标记为成功')
      }
    }
  ...
  private async resumeArticlePublishingAfterHandoffNow(
    ...
    const recoveryOperation = this.createArticlePublishingOperation(
      'recovery.restore-exact-draft',
      'open-editor',
      'studio',
      { id: found.attempt.id, executionGeneration, launchOperationId },
      now,
      '用户已完成人工卡点，准备从草稿管理页恢复原草稿',
      '核验原账号、draftId、标题和保存状态',
    )
    const recoveryProtocol = this.appendArticlePublishingTransition(publishing, {
      operationRunId: recoveryOperation.operationRunId,
      kind: 'recovery-started',
      occurredAt: now,
    ...
    const executionProtocol = runtimeOperation
      ? {
          ...this.appendArticlePublishingTransition(publishing, {
            ...
            kind: 'runtime-prepare-started',
            occurredAt: now,
            summary: '开始准备第一次页面检查 Runtime',
          }),
  ...
  private async bindArticlePublishingRuntimeNow(
    ...
    // The final Runtime handshake has just reread the same saved draft on its exact Page.
    // Only URLs already bound by main's one-file observer can resolve interrupted uploads.
    if (
      recoveryVerification?.imageEnumerationComplete &&
      publishingForBind.draft?.recovery?.status === 'verified'
    ) {
        ...
        publishing.assets
          .filter(
            (asset) =>
              ...
              asset.platformUrl &&
              recoveryVerification.images?.some(
      ...
      publishingForBind = {
                ...
                uploadAttempts: asset.uploadAttempts.map((upload, index) =>
                  index === asset.uploadAttempts.length - 1
                    ? {
                        ...
                        evidence: [
                          ...upload.evidence,
                          `恢复同账号原稿 ${recoveryVerification.draftId}；已保存正文中的 ${asset.platformUrl} 加载成功`,
                        ].slice(-40),
                      }
                    : upload,
                ),
        ...
        sideEffects: publishingForBind.sideEffects.map((effect) =>
          ...
          effect.executionGeneration < executionGeneration &&
          effect.kind === 'upload-asset' &&
          recoveredIds.has(effect.targetId.replace(/:attempt-\d+$/u, '')) &&
          ['dispatched', 'result-unknown'].includes(effect.status)
            ? { ...effect, status: 'verified', observedAt: now }
            : effect,
        ),
      }
    }
    if (recovery?.executionGeneration === executionGeneration) {
      if (
        publishingForBind.assets.some((asset) =>
          ['result-unknown', 'reconciling'].includes(asset.status),
        ) ||
        publishingForBind.sideEffects.some(
          (effect) =>
            effect.executionGeneration < executionGeneration &&
            effect.kind === 'save-draft' &&
            ['reserved', 'dispatched', 'result-unknown'].includes(effect.status),
    ...
    const runtimeSnapshot: ArticlePublishingRuntimeSnapshot = {
      tabId: browserTask.tabId,
      browserViewRuntimeGeneration: browserTask.browserViewRuntimeGeneration,
      webContentsId: browserTask.webContentsId,
      playwrightConnectionGeneration: browserTask.playwrightConnectionGeneration,
    ...
    publishingForBind = setArticlePublishingPlanResult(publishingForBind, {
      id: 'runtime.ready',
      status: 'completed',
      observedAt: now,
      generation: executionGeneration,
      evidence: `Agent ${agent.agentRunId} · BrowserTask ${browserTask.browserTaskRunId} · Tab ${browserTask.tabId} · View g${browserTask.browserViewRuntimeGeneration} · WebContents ${browserTask.webContentsId} · Page g${browserTask.playwrightPageBindingGeneration}`,
    })
    ...
    const firstInspectOperation =
      runtimeOperation?.definitionId === 'runtime.prepare-first-inspect'
        ? this.createArticlePublishingOperation(
            'page.first-inspect',
            publishingForBind.execution.currentStepId ?? 'open-editor',
            'agent',
            nextAttempt,
            now,
            'Agent、BrowserTask 和当前 Page 已完成主进程绑定',
            '在当前精确 Page 上完成第一次只读检查',
            'ready',
            runtimeSnapshot,
          )
        : undefined
      ...
      for (const transition of [
        {
          ...
          summary: '账号写入租约已转交当前 BrowserTask',
        },
        {
          kind: 'binding-committed' as const,
          summary: 'Agent、BrowserTask、Tab 和 Page binding 已持久化',
        },
  ...
  private async rebindArticlePublishingBrowserRuntimeNow(input: {
    ...
    const nextRuntime: ArticlePublishingRuntimeSnapshot = {
      tabId: input.tabId,
      browserViewRuntimeGeneration: input.browserViewRuntimeGeneration,
      webContentsId: input.webContentsId,
      playwrightConnectionGeneration: input.playwrightConnectionGeneration,
    ...
    let executionProtocol = currentOperation
      ? this.appendArticlePublishingTransition(publishing, {
          ...
          occurredAt: now,
          summary: '当前 Page Runtime 已改代，正在重新核验原草稿',
  ...
  private async reconcileArticlePublishingRuntimeNow(
    ...
    if (
      TERMINAL_ATTEMPT_STATUSES.has(found.attempt.status) &&
      !(input.source === 'user-cancel' && found.attempt.status === 'interrupted')
    ) {
      return { success: true, data: structuredClone(found.affair) }
    }
    ...
    // A checkpoint handoff has already closed its leases and recorded the real
    // blocker. The ensuing Agent/Browser completion must not overwrite it.
    if (
      found.attempt.status === 'waiting-human' &&
      matchedBinding?.status === 'terminal' &&
      ['agent-terminal', 'browser-terminal'].includes(input.source)
    )
      return { success: true, data: structuredClone(found.affair) }
    ...
    const requestedTarget: ArticlePublishingLifecycleTarget =
      input.source === 'user-cancel'
        ? 'cancelled'
        : ['agent-terminal', 'browser-terminal', 'tab-lost'].includes(input.source)
          ? 'interrupted'
  ...
  private validateArticlePublishingReporter(
    ...
    if (reporter.trustedPageEvidence?.isCurrent && !reporter.trustedPageEvidence.isCurrent()) {
      return '发布回报的页面证据在进入状态事务前已过期'
    }
    if (
  ...
  private async reportArticlePublishingCheckpointNow(
    ...
    const reporterError = this.validateArticlePublishingReporter(found, reporter)
    if (reporterError) return this.transitionError(reporterError)
    if (publishing.executionProtocol.current) {
      return this.transitionError(
        '当前 operation 尚未完成；只允许执行它签发的页面检查，不能推进业务 checkpoint',
      )
    }
    ...
    if (checkpoint.status === 'completed' && input.status === 'completed') {
      return { success: true, data: structuredClone(found.affair) }
    }
    ...
    // Recovery only settled the old save lock. A fresh complete body comparison
    // can now verify that dispatched write without manufacturing another write.
    const recoveredBodyEffect =
      input.stepId === 'fill-body' &&
      ...
      const verifiedAutosave =
        ...
        !eligibleEffects.some(
          ...
          ? eligibleEffects.find(
              (effect) =>
                effect.kind === 'save-draft' &&
                effect.targetId.startsWith('autosave:') &&
                effect.status === 'verified',
            )
          : undefined
    ...
    const sideEffects =
      input.status === 'completed' && input.stepId === 'save-draft'
        ...
        : input.status === 'completed' && input.stepId === 'fill-fields'
          ...
          : input.status === 'completed' && input.stepId === 'fill-body'
            ? this.updateLatestSideEffect(
                publishing,
                found.attempt,
                (effect) =>
    ...
    let detailedPublishing = publishing
    if (input.status === 'completed' && reporter.trustedPageEvidence) {
      const id =
        input.stepId === 'save-draft'
          ? 'save.verify'
          : input.stepId === 'verify-publication'
            ? 'publication.verify'
            : undefined
      if (id)
      ...
      if (
        input.stepId === 'save-draft' &&
        !eligibleEffects.some((e) => e.targetId.startsWith('manual-save:') && e.dispatchedAt)
      )
        detailedPublishing = setArticlePublishingPlanResult(detailedPublishing, {
          id: 'save.dispatch',
          status: 'skipped',
    ...
    const baseAffair: WebAffair = {
      ...
      articlePublishing: {
        ...
        checkpoints: checkpoints.map((c) => ({
          ...c,
          details: detailedPublishing.checkpoints.find((d) => d.stepId === c.stepId)?.details,
        })),
        sideEffects,
        publication,
        execution: {
  ...
  private async recordArticlePublishingDraftAnchorNow(
    ...
    const creationEffect =
      ...
      publishing.sideEffects.find(
        (effect) =>
          effect.key === creation.sideEffectKey &&
          effect.kind === 'save-draft' &&
          effect.targetId === 'initial-draft:save' &&
          effect.attemptId === attemptId &&
          effect.executionGeneration === executionGeneration &&
    ...
    if (
      creation &&
      (!creationEffect ||
        !creation.platformAccountId ||
        creation.normalizedTitle !== publishing.fields.title.replace(/\s+/gu, ' ').trim())
    ) {
      return this.transitionError('首次草稿响应缺少本次已派发保存授权或账号标题证据')
    }
    if (
      ...
      !(
        ...
        // A late response records only the exact ID, never progress or new action permission.
        // The generation checks above still reject responses from a replaced execution.
        (Boolean(creationEffect) &&
          found.attempt.browserTaskRunId === browserTaskRunId &&
          ['cancelled', 'interrupted', 'result-unknown', 'waiting-human', 'failed'].includes(
    ...
    return this.persistAffair({
      ...
      articlePublishing: {
        ...
        draft: {
          ...
          ...(creation
            ? {
                platformAccountId: creation.platformAccountId,
                normalizedTitle: creation.normalizedTitle,
              }
            : {}),
          lastVerifiedAt: now,
        },
  ...
  private createArticlePublishingOperation(
    definitionId: ArticlePublishingCurrentOperation['definitionId'],
    checkpointId: ArticlePublishingCurrentOperation['checkpointId'],
    owner: ArticlePublishingCurrentOperation['owner'],
    attempt: Pick<WebAffairAttempt, 'id' | 'executionGeneration' | 'launchOperationId'>,
    now: string,
    startSummary: string,
    goalSummary: string,
    status: ArticlePublishingCurrentOperation['status'] = 'running',
    runtime?: ArticlePublishingRuntimeSnapshot,
  ): ArticlePublishingCurrentOperation {
    return {
  ...
  private appendArticlePublishingTransition(
    publishing: ArticlePublishingState,
    transition: Omit<ArticlePublishingOperationTransition, 'id'>,
  ): ArticlePublishingState['executionProtocol'] {
    return {
      ...publishing.executionProtocol,
      recentTransitions: [
        ...publishing.executionProtocol.recentTransitions,
        { id: randomUUID(), ...transition },
      ].slice(-ARTICLE_OPERATION_TRANSITION_LIMIT),
    }
  }
  ...
  private async transitionArticlePublishingFirstInspectNow(
    ...
    if (
      ...
      return this.transitionError('首次页面检查不属于当前执行代次')
    }
    const current = publishing.executionProtocol.current
    if (!current || current.definitionId !== 'page.first-inspect') {
      return this.transitionError('当前执行义务不是首次页面检查')
    }
    if (
      current.operationRunId !== input.expectedOperationRunId ||
      current.revision !== input.expectedOperationRevision ||
      current.attemptId !== input.attemptId ||
      current.executionGeneration !== input.executionGeneration ||
      current.launchOperationId !== input.launchOperationId
    ) {
    ...
    if (!completed && !['ready', 'running'].includes(current.status)) {
      return this.transitionError('首次页面检查当前不可启动')
    }
    if (completed && current.status !== 'running') {
      return this.transitionError('首次页面检查尚未开始或已经结束')
    }
    const recovery = publishing.draft?.recovery
    const exactRecoveredDraftProven =
      !recovery ||
      recovery.executionGeneration !== input.executionGeneration ||
      (input.pageKind === 'editor' &&
        input.draftId === recovery.expectedDraftId &&
    ...
    const transition: Omit<ArticlePublishingOperationTransition, 'id'> = {
      operationRunId: current.operationRunId,
      kind: completed ? 'first-inspect-completed' : 'first-inspect-started',
      occurredAt: now,
      summary: completed
        ? 'Agent 第一次页面检查已由当前 Page 适配器验证'
        : 'Agent 已开始第一次只读页面检查',
      currentRuntime: input.runtime,
    }
    const protocol = this.appendArticlePublishingTransition(publishing, transition)
    const ownsBusinessCheckpoint = ['open-editor', 'verify-account'].includes(current.checkpointId)
    const checkpoints =
      completed && ownsBusinessCheckpoint
        ? publishing.checkpoints.map((checkpoint) =>
            checkpoint.stepId === current.checkpointId
              ? {
                  ...checkpoint,
                  status: 'completed' as const,
                  ...
                  evidence: [
                    ...checkpoint.evidence,
                    `main adapter 已在当前精确 Page 完成首次只读核验：${input.pageKind ?? 'unknown'}`,
                  ].slice(-40),
                }
              : checkpoint,
          )
    ...
    const nextCheckpoint =
      completed && ownsBusinessCheckpoint
        ...
        : undefined
    const nextOperation =
      completed && nextCheckpoint?.stepId === 'verify-account'
        ? this.createArticlePublishingOperation(
            'page.first-inspect',
            'verify-account',
            'agent',
            found.attempt,
            now,
            '编辑页已确认，等待当前 Agent 复核平台账号',
            '在同一精确 Page 上读回真实 CSDN 账号',
            'ready',
            input.runtime,
          )
        : undefined
    return this.persistAffair({
      ...
      articlePublishing: {
        ...
        execution: completed
          ? {
              ...publishing.execution,
              currentStepId: ownsBusinessCheckpoint
                ? nextCheckpoint?.stepId
                : publishing.execution.currentStepId,
            }
          : publishing.execution,
      },
      events: completed
        ? this.appendEvent(
            ...
            this.event(
              'node-status-changed',
              `main 已完成 ${current.checkpointId} 的当前 Page 首次只读核验`,
              now,
              { nodeId: found.attempt.nodeId, attemptId: found.attempt.id },
            ),
          )
  ...
  private articlePublishingRuntimeSnapshotsEqual(
  ...
  private async failArticlePublishingCurrentOperationNow(input: {
    workspaceId: string
    affairId: string
    attemptId: string
    executionGeneration: number
    ...
    const now = this.timestamp()
    const protocol = this.appendArticlePublishingTransition(publishing, {
      operationRunId: current.operationRunId,
  ...
  private async verifyArticlePublishingRecoveryNow(
    ...
    const currentOperation = publishing.executionProtocol.current
    const runtimeOperation =
      currentOperation?.definitionId === 'recovery.restore-exact-draft'
        ? this.createArticlePublishingOperation(
            'runtime.prepare-first-inspect',
            'open-editor',
            'studio',
            found.attempt,
            now,
            '原草稿已核验，正在建立 Agent 与 BrowserTask Runtime',
            '使用最新 Page 身份完成绑定并开放第一次只读检查',
            'running',
            runtimeSnapshot,
          )
        : currentOperation
    let executionProtocol = publishing.executionProtocol
    if (currentOperation?.definitionId === 'recovery.restore-exact-draft' && runtimeOperation) {
      executionProtocol = this.appendArticlePublishingTransition(publishing, {
        operationRunId: currentOperation.operationRunId,
        kind: 'draft-restored',
        occurredAt: now,
        summary: `已核验原草稿 ${input.draftId}`,
        currentRuntime: runtimeSnapshot,
      })
      executionProtocol = this.appendArticlePublishingTransition(
        { ...publishing, executionProtocol },
    ...
    } else if (runtimeOperation?.definitionId === 'runtime.prepare-first-inspect') {
      executionProtocol = this.appendArticlePublishingTransition(publishing, {
        operationRunId: runtimeOperation.operationRunId,
        kind: 'draft-reverified',
        occurredAt: now,
        summary: `Page 改代后已重新核验原草稿 ${input.draftId}`,
        previousRuntime: runtimeOperation.runtime,
        currentRuntime: runtimeSnapshot,
      })
    }
    ...
    const reconciledSideEffects = publishing.sideEffects.map((effect) =>
      effect.attemptId === input.attemptId &&
      effect.executionGeneration < input.executionGeneration &&
      effect.kind === 'save-draft' &&
      ['reserved', 'dispatched', 'result-unknown'].includes(effect.status)
        ? // 只证明重新打开的原草稿当前已保存，不证明旧动作的预期正文已保存。
          // 解除历史保存锁，但不完成任何业务检查点；未完成步骤仍须执行和核验。
          { ...effect, status: 'reconciled' as const, observedAt: now }
        : effect,
    )
    return this.persistAffair({
      ...found.affair,
      articlePublishing: {
        ...publishing,
        sideEffects: reconciledSideEffects,
        executionProtocol: {
          ...executionProtocol,
          ...(runtimeOperation
            ? {
                current: {
                  ...runtimeOperation,
                  revision: runtimeOperation.revision + 1,
                  status: 'running' as const,
                  lastTransitionAt: now,
  ...
  private async reportArticlePublishingAssetNow(
    ...
    if (publishing.executionProtocol.current) {
      return this.transitionError(
        '当前 operation 尚未完成；只允许执行它签发的页面检查，不能推进图片状态',
      )
    }
    if (publishing.execution.currentAttemptId !== input.attemptId) {
      return this.transitionError('报告的 Attempt 不是当前发布事务')
    ...
    return this.persistAffair({
      ...
      articlePublishing: {
        ...
        sideEffects: publishing.sideEffects.map((candidate) =>
          candidate.key === sideEffectKey ? { ...candidate, consumedAt: now } : candidate,
        ),
      },
      events: this.appendEvent(
        found.affair,
  ...
  private async dispatchArticlePublishingSideEffectNow(
    affairId: string,
    attemptId: string,
    executionGeneration: number,
    sideEffectKey: string,
    ...
    if (
      ...
      return this.transitionError('网页副作用派发身份已被取消、接管或改代')
    }
    const now = this.timestamp()
    return this.persistAffair({
      ...found.affair,
      ...
      events: this.appendEvent(
        ...
        this.event(
          'node-status-changed',
          `即将派发网页副作用：${effect.kind}/${effect.targetId}`,
          now,
          { nodeId: found.attempt.nodeId, attemptId },
        ),
      ),
      updatedAt: now,
    })
  ...
  private reduceArticlePublishingLifecycle(
    ...
    const closesExecution = isTerminal || target === 'waiting-human'
    const currentStepId = publishing.execution.currentStepId
    const nextAttempt: WebAffairAttempt = {
      ...attempt,
      status: attemptStatus,
      runtimeBindings: closesExecution
    ...
    const checkpoints = publishing.checkpoints.map((checkpoint) => {
      ...
      if (
        (isRuntimeLoss || target === 'cancelled') &&
        ['running', 'waiting-platform', 'verifying'].includes(checkpoint.status)
      ) {
    ...
    const sideEffects = publishing.sideEffects.map((effect) =>
      closesExecution && effect.status === 'dispatched'
        ? { ...effect, status: 'result-unknown' as const, observedAt: now }
        : closesExecution && effect.status === 'reserved'
          ? { ...effect, status: 'rejected' as const, observedAt: now }
          : effect,
    )
    const publication =
    ...
    if (
      closesExecution &&
      currentOperation &&
      !['interrupted', 'result-unknown', 'failed'].includes(currentOperation.status)
    ) {
      ...
      executionProtocol = this.appendArticlePublishingTransition(publishing, {
        ...
        occurredAt: now,
        summary: reason,
        ...(currentOperation.runtime ? { currentRuntime: currentOperation.runtime } : {}),
      })
      executionProtocol = {
        ...executionProtocol,
        current:
          target === 'published'
            ...
            : {
                ...
                owner: target === 'waiting-human' ? ('human' as const) : currentOperation.owner,
                lastTransitionAt: now,
              },
      }
    }
    const draft =
      closesExecution && publishing.draft?.recovery?.writePermit
        ? {
            ...publishing.draft,
            recovery: {
              ...publishing.draft.recovery,
  ...
  private async persistAffair(affair: WebAffair): Promise<WebAffairOperationResult<WebAffair>> {
    ...
    if (affair.articlePublishing)
      affair = {
        ...affair,
        articlePublishing: foldArticlePublishingPlanResults(
          affair.articlePublishing,
          affair.updatedAt,
        ),
      }
