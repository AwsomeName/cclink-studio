...
describe('editor Agent scope', () => {
  ...
  it('keeps the read-only account catalog available in every focused scope', () => {
    ...
    expect(scopeToAllowedTools(browserScope)).toContain('mcp__cclink_studio__web_account_open')
    expect(toolBelongsToScope('web_account_open', browserScope)).toBe(true)
    expect(toolBelongsToScope('web_account_open', androidScope)).toBe(false)
    expect(toolBelongsToScope('web_account_open', { kind: 'editor' })).toBe(false)
  })
})
